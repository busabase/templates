#!/usr/bin/env node
// Trusted hand-off step. Busa Ads' AirApp never pulls ad-platform reports
...
// persisted content/busa-ads-app/.data/ads_snapshot.json to Busabase's platforms/campaigns/
...
// content/busa-ads-app/app/js/ads-model.js's buildSnapshot()/recomputeDerived(), so this
...
import { appConfig } from "../content/busa-ads-app/app/js/config.js";
...
function fail(message) {
  console.error(`busa-ads ingest: ${message}`);
  process.exitCode = 1;
...
async function main() {
  ...
  if (!resources.folder || resources.missing.length) {
    throw new Error("Busa Ads Busabase resources are not provisioned yet; run the AirApp setup first.");
  }
