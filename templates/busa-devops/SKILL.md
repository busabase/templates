---
name: busa-devops
description: Product-fleet ops desk (Busabase App-in-Skill) for uptime/service health, SSL certificate expiry, domain renewal, API key rotation, cloud spend anomalies, and a review queue of agent-proposed action cards. Use when the user invokes $busa-devops or /busa-devops, or asks about uptime, service health checks, SSL certificate expiry, domain renewal, domain expiry, API key rotation, token rotation reminders, cloud spend anomaly, billing spikes, ops desk, incident review, or reviewing agent-proposed action cards (renew domain, rotate key, investigate spend, restart service, acknowledge incident).
metadata:
  category: platform
  tags:
    - risk:gated-write
...
<table>
  <tr>
    <td width="50%"><img src="assets/screenshots/overview.webp" alt="Busa DevOps overview"></td>
    <td width="50%"><img src="assets/screenshots/actions.webp" alt="Busa DevOps action queue"></td>
  </tr>
  <tr>
  ...
  <tr>
    <td width="50%"><img src="assets/screenshots/expiries.webp" alt="Busa DevOps expiry ledger"></td>
    <td width="50%"><img src="assets/screenshots/services.webp" alt="Busa DevOps services"></td>
  </tr>
  <tr>
...
Use this skill as the operator's product-fleet operations desk. It watches the
...
1. Read and follow `$kelly-app-skill-creator` for product behavior, visual
   quality, responsive layout, and the complete canonical `content/busa-devops-app/` artifact.
...
- Checks are read-only network probes of the operator's own endpoints and domains: HTTP GET for status/latency, a TLS handshake for certificate expiry, and public RDAP lookups for domain expiry. Nothing else is contacted.
...
Seven Bases under one application Folder (`busa-devops`), declared in
`content/busa-devops-app/app/js/config.js` and the generated template sidecars under `content/`:
...
```bash
node skills/busa-devops/scripts/check_services.mjs roster.json --apply
node skills/busa-devops/scripts/sync_domains.mjs roster.json --apply
```

## Local App

Default behavior is AirApp-first — give the user the clickable AirApp URL.
Start `pnpm --dir content/busa-devops-app dev` only when local preview/debugging is explicitly
requested.
...
node skills/busa-devops/scripts/check_services.mjs roster.json --apply
node skills/busa-devops/scripts/sync_domains.mjs roster.json --apply
node skills/busa-devops/scripts/ingest_spend.mjs payload.json --apply
node skills/busa-devops/scripts/execute_decisions.mjs
node skills/busa-devops/scripts/execute_decisions.mjs --complete <action_id> --note "..."
pnpm --dir skills/busa-devops/content/busa-devops-app dev
```

In normal use, invoke `/busa-devops`, let the skill run the checks that are
