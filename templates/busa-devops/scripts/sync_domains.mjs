...
// changed, from content/busa-devops-app/.data/ops_snapshot.json to Busabase's Expiries Base.
//
...
import { appConfig } from "../content/busa-devops-app/app/js/config.js";
...
async function upsertRow(client, declared, existing, idField, idValue, fields, message, apply) {
  ...
  if (existing) {
    await client.records.changeRequest({
      ...
      fields: normalized,
      message,
      author: "busa-devops-sync-domains",
  ...
  await client.bases.createChangeRequest({
    ...
    fields: normalized,
    message,
    submittedBy: "busa-devops-sync-domains",
    autoMerge: true,
  });
  return "created";
}
...
async function rdapExpiration(domain) {
  const res = await fetch(`https://rdap.org/domain/${encodeURIComponent(domain)}`, {
    headers: { accept: "application/rdap+json, application/json", "user-agent": "busa-devops-check/1.0" },
    redirect: "follow",
...
async function main() {
  ...
  if (!resources.folder || resources.missing.length) {
    throw new Error("Busa DevOps Busabase resources are not provisioned yet; run the AirApp setup first.");
  }
