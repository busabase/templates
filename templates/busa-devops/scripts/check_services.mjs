...
// target changed, from content/busa-devops-app/.data/ops_snapshot.json to Busabase's
...
async function upsertRow(client, declared, existing, idField, idValue, fields, message, apply) {
  ...
  if (existing) {
    await client.records.changeRequest({
      ...
      fields: normalized,
      message,
      author: "busa-devops-check-services",
  ...
  await client.bases.createChangeRequest({
    ...
    fields: normalized,
    message,
    submittedBy: "busa-devops-check-services",
    autoMerge: true,
  });
  return "created";
}
...
async function probeHttp(url, degradedLatencyMs) {
  ...
  try {
    const res = await fetch(url, {
      method: "GET",
      redirect: "follow",
      headers: { "user-agent": "busa-devops-check/1.0" },
