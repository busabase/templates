---
name: busa-lead-funnel
description: Busabase-backed App-in-Skill control panel / kanban board for a BD or sourcing team triaging merchant and business financing leads for a lender or investment fund. Use when the user invokes $busa-lead-funnel or /busa-lead-funnel, wants to review the deal sourcing pipeline, funnel, lead board, or asks to move a lead's stage, reject a lead, add a note, score a lead, or see funnel conversion rates. Deterministic rule-based lead scoring only — never an LLM call — and never sends outreach, signs term sheets, or moves money.
metadata:
  category: rbf
  tags:
    - risk:local-write
...
Busa Lead Funnel is a Busabase Cloud App-in-Skill. Its canonical product
...
rule-based function (`content/busa-lead-funnel-app/app/js/lead-funnel-model.js`, ported from the
...
`busa-crm`'s kanban stage moves work.
...
1. Read and follow `$kelly-app-skill-creator` for product behavior, visual
   quality, responsive layout, and the complete canonical `content/busa-lead-funnel-app/` artifact.
...
- Deterministic, rule-based lead-quality score only
  (`content/busa-lead-funnel-app/app/js/lead-funnel-model.js`). NEVER call an LLM to score, rank, or
  auto-reject a lead.
...
Two Bases under one application Folder (`busa-lead-funnel`), declared in
`content/busa-lead-funnel-app/app/js/config.js` and the generated template sidecars under `content/`:
...
`content/busa-lead-funnel-app/app/js/lead-funnel-model.js` (`scoreLead`) computes a deterministic
...
`busabase-sdk`, exactly like `busa-crm`'s kanban stage moves — there is no
...
Start `pnpm --dir content/busa-lead-funnel-app dev` only when local preview/debugging is explicitly
requested.
...
- the skill contains the complete canonical `content/busa-lead-funnel-app/` project and
  `pnpm --dir content/busa-lead-funnel-app dev` remains supported;
...
- `pnpm --dir content/busa-lead-funnel-app run check` and `node --test` pass.
