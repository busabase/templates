# Busa Lead Funnel Schema

Use this schema when reading or writing Busa Lead Funnel's Busabase Bases.
Field slugs are kebab-case in Busabase and normalized to snake_case in app
code (`content/busa-lead-funnel-app/app/js/providers/busabase-provider.js`,
`content/busa-lead-funnel-app/app/js/lead-funnel-model.js`). `score`, `score_breakdown`, and
...
## Settings (`busa-lead-funnel-settings`)

One row per `kind`, looked up by `record-id`:

| `record-id` | `kind` | `payload` (JSON) |
| --- | --- | --- |
| `busa-lead-funnel-config` | `config` | `{base_currency, fund_profile: {display_name, product, target_check_size}, scoring_criteria: {ideal_store_count_min, ideal_store_count_max, ideal_monthly_revenue_min, ideal_monthly_revenue_max, low_risk_categories, medium_risk_categories, higher_risk_categories}}` |

If no `config` row exists, the app falls back to `DEFAULT_SCORING_CRITERIA`
(`content/busa-lead-funnel-app/app/js/lead-funnel-model.js`) and an empty `fund_profile` — the board
