---
name: busa-radar
description: The operator's market-intelligence desk (Busabase App-in-Skill). Merges passive competitor monitoring (Radar), agent-run deep research with approved briefs and cited reports (Research), and keyword/topic demand signals (Trends) over one Busabase-backed snapshot, watchlist, and review model. Use when the user invokes $busa-radar or /busa-radar, or asks for competitor monitoring, market intelligence, market research, research reports, trends, rising keywords, pricing changes, product launches, changelog diffs, competitor reviews, funding or hiring news, or opportunity triage.
metadata:
  category: growth
  tags:
    - risk:local-write
...
<table>
  <tr>
    <td width="50%"><img src="assets/screenshots/overview.webp" alt="Busa Radar overview"></td>
    <td width="50%"><img src="assets/screenshots/research.webp" alt="Busa Radar research desk"></td>
  </tr>
  <tr>
  ...
  <tr>
    <td width="50%"><img src="assets/screenshots/signals.webp" alt="Busa Radar competitor signals"></td>
    <td width="50%"><img src="assets/screenshots/trends.webp" alt="Busa Radar trends"></td>
  </tr>
  <tr>
...
Use this skill as the operator's market-intelligence desk. One Busabase-backed
...
1. **Radar (passive monitoring)**: the agent periodically checks a watchlist
   ...
   highlights via `scripts/ingest_signals.mjs`. The operator triages each signal in
   the app: act / watch / ignore / needs info.
2. **Research (active deep dives)**: the operator queues research questions; the
   ...
   via `scripts/file_report.mjs`. The operator reads, annotates, rates confidence,
...
1. Read and follow `$kelly-app-skill-creator` for product behavior, visual
   quality, responsive layout, and the complete canonical `content/busa-radar-app/` artifact.
...
- Collection and research are read-only over public pages and the operator's own
...
Nine Bases under one application Folder (`busa-radar`), declared in
`content/busa-radar-app/app/js/config.js` and the generated template sidecars under `content/`:
...
and positioning (what the operator sells, to whom, and against whom), watch
...
```bash
node skills/busa-radar/scripts/ingest_signals.mjs payload.json
```
...
Start `pnpm --dir content/busa-radar-app dev` only when local preview/debugging is explicitly
requested.
...
5. The operator triages in the app: Act = approve (queues the handoff), Watch = leave in review with a note, Ignore = done, Needs info = blocked.

## Research Workflow

1. The operator asks a question (in chat or as a follow-up in the app). File it with a drafted brief: `node scripts/file_report.mjs <payload.json>` with a `brief` block — scope, planned sources, depth, expected deliverable. The question enters `brief_needs_review`; include `payload.question` to create it if it doesn't exist yet.
2. The operator reviews the brief in `#/research/<id>`: approve (question moves to `researching`), request changes, or block.
...
5. The operator reads, annotates, rates confidence (0-5), and asks follow-ups in the app; follow-ups are appended onto the question's `followups[]` field for another research round.
...
```bash
node skills/busa-radar/scripts/ingest_signals.mjs payload.json
node skills/busa-radar/scripts/ingest_trends.mjs payload.json [kelly-seo-snapshot.json]
node skills/busa-radar/scripts/file_report.mjs payload.json
node skills/busa-radar/scripts/execute_decisions.mjs
node skills/busa-radar/scripts/execute_decisions.mjs --apply
pnpm --dir skills/busa-radar/content/busa-radar-app dev
```

In normal use, invoke `/busa-radar`, let the skill collect/research what's
