...
Use this schema when reading or writing Busa Radar's Busabase Bases. Field
slugs are kebab-case in Busabase and normalized to snake_case in app code
(`content/busa-radar-app/app/js/providers/busabase-provider.js`, `content/busa-radar-app/app/js/radar-model.js`).
...
## Watchlist (`busa-radar-watchlist`)
...
## Research Questions (`busa-radar-questions`)
...
## Research Briefs (`busa-radar-briefs`)
...
The agent drafts the brief first; research starts only after the operator approves
...
## Research Reports (`busa-radar-reports`)
...
| `confidence` | `confidence` | number | the operator's 0-5 rating, set through the app |
...
## Trend Movers (`busa-radar-movers`)
...
## Opportunities (`busa-radar-opportunities`)
...
| `actor` | `actor` | text | `busa-radar-agent` |
