...
// records instead of content/busa-radar-app/.data/radar_snapshot.json.
//
...
import { appConfig } from "../content/busa-radar-app/app/js/config.js";
import { MOVER_SOURCES } from "../content/busa-radar-app/app/js/radar-model.js";
...
async function create(client, declared, fields, message) {
  return client.bases.createChangeRequest({
    ...
    message,
    submittedBy: "busa-radar-ingest-trends",
    autoMerge: true,
  });
}
...
async function update(client, existing, fields, message) {
  return client.records.changeRequest({
    ...
    fields: toBusabaseFields(fields),
    message,
    author: "busa-radar-ingest-trends",
...
async function main() {
  ...
  if (!resources.folder || resources.missing.length) {
    throw new Error("Busa Radar Busabase resources are not provisioned yet; run the AirApp setup first.");
  }
  ...
  await create(
    client,
    declared("sync-log"),
    { log_id: `log-${Date.now().toString(36)}`, at: now, actor: "busa-radar-agent", action: "ingest_trends", detail },
