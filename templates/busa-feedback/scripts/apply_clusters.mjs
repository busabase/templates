...
// only the storage target changed, from content/busa-feedback-app/.data/feedback_snapshot.json to
...
import { appConfig } from "../content/busa-feedback-app/app/js/config.js";
import { REQUEST_STATUSES, TRENDS } from "../content/busa-feedback-app/app/js/feedback-model.js";
...
function fail(message) {
  console.error(`busa-feedback apply_clusters: ${message}`);
  process.exit(1);
}
...
async function upsertRow(client, declared, existing, fields, message, apply) {
  ...
  await client.bases.createChangeRequest({
    ...
    fields: normalized,
    message,
    submittedBy: "busa-feedback-cluster",
    autoMerge: true,
  });
  return "created";
}
...
async function main() {
  ...
  if (!resources.folder || resources.missing.length) {
    throw new Error("Busa Feedback Busabase resources are not provisioned yet; run the AirApp setup first.");
  }
