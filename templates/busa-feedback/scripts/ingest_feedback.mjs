...
// content/busa-feedback-app/.data/feedback_snapshot.json to Busabase's products/sources/feedback
...
function fail(message) {
  console.error(`busa-feedback ingest: ${message}`);
  process.exit(1);
}
...
async function upsertRow(client, declared, existing, fields, message, apply) {
  ...
  await client.bases.createChangeRequest({
    ...
    fields: normalized,
    message,
    submittedBy: "busa-feedback-ingest",
    autoMerge: true,
  });
  return "created";
}
