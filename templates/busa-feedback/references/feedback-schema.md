# Busa Feedback Schema

Use this schema when reading or writing Busa Feedback's Busabase Bases.
Field slugs are kebab-case in Busabase and normalized to snake_case in app
code (`content/busa-feedback-app/app/js/providers/busabase-provider.js`, `content/busa-feedback-app/app/js/feedback-model.js`).
...
## Products (`busa-feedback-products`)
...
## Feedback (`busa-feedback-feedback`)
...
## Requests (`busa-feedback-requests`)
...
## Proposals (`busa-feedback-proposals`)
...
| `review-note` | `review_note` | longtext | the operator's note from the review UI |
| `created-at` | `created_at` | text | ISO timestamp |
