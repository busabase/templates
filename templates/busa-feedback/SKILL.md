...
<table>
  <tr>
    <td width="50%"><img src="assets/screenshots/overview.webp" alt="Busa Feedback overview"></td>
    <td width="50%"><img src="assets/screenshots/inbox.webp" alt="Busa Feedback inbox"></td>
  </tr>
  <tr>
  ...
  <tr>
    <td width="50%"><img src="assets/screenshots/requests.webp" alt="Busa Feedback requests"></td>
    <td width="50%"><img src="assets/screenshots/roadmap.webp" alt="Busa Feedback roadmap decisions"></td>
  </tr>
  <tr>
...
Use this skill as the operator's voice-of-customer desk. It aggregates raw user
...
decision queue for the operator's verdict.

Division of labor: the skill (scripts + agent) collects, clusters, and
drafts; the app is where the operator triages feedback and decides on proposals;
...
busa-messenger or busa-email). Busa Feedback sits downstream of
busa-email, busa-messenger, and busa-social: those skills' agents hand
...
1. Read and follow `$kelly-app-skill-creator` for product behavior, visual
   quality, responsive layout, and the complete canonical `content/busa-feedback-app/` artifact.
...
- Any outbound side effect — replying to a user, publishing a changelog note, editing a public roadmap — is approval-required and executed by the agent via other skills (busa-messenger, busa-email, docs edits) only after the matching proposal is approved and `scripts/execute_decisions.mjs --apply` marks it `handoff_ready`. `scripts/execute_decisions.mjs` never sends anything itself.
- Own-community data only: ingest feedback addressed to the operator's own products from the operator's own channels and accounts via `scripts/ingest_feedback.mjs`. Do not scrape third-party communities or collect data about other companies' users.
- Treat feedback as user PII-adjacent. Never commit real tokens, raw platform exports, or Busabase credentials.

## Busabase Resources

Eight Bases under one application Folder (`busa-feedback`), declared in
`content/busa-feedback-app/app/js/config.js` and the generated template sidecars under `content/`:
...
Start `pnpm --dir content/busa-feedback-app dev` only when local preview/debugging is explicitly
requested.
...
notes — hands feedback to Busa Feedback the same way:
...
2. Run `node skills/busa-feedback/scripts/ingest_feedback.mjs <payload.json> [more.json ...] --apply`.
...
Typical handoffs: busa-email exports support threads mentioning features;
busa-messenger exports Discord/Slack community posts; busa-social exports
...
3. Write a cluster-assignment payload (shape in `references/feedback-schema.md`) and run `node skills/busa-feedback/scripts/apply_clusters.mjs <assignments.json> --apply`. The script validates ids, upserts request drafts, links feedback, and logs the run. Request `frequency`/`weighted_score` recompute automatically client-side — the script never writes them.
4. When clusters warrant action, draft proposals directly into the `proposals` Base (promote/decline/merge/changelog) with reason, evidence, and an editable public draft, then send the operator to `#/roadmap`.

## Roadmap Decision Workflow

1. The operator reviews `#/roadmap`: edits drafts, writes review notes, and clicks Approve / Request changes / Block. The app writes the verdict directly onto the proposal record (`status`/`review-note`/`draft`/`decided-at`) through `busabase-sdk` — never a separate decisions file. From a standalone local preview the write merges immediately (trusted operator); from the deployed AirApp it creates a pending ChangeRequest for the trusted process to merge.
2. Before executing, run `node skills/busa-feedback/scripts/execute_decisions.mjs` (dry-run) to see the plan for every `approved` proposal: `update_roadmap` (target lane) and `merge_requests` are LOCAL operations; `publish_changelog_note` (draft id) and `send_decline_reply` (handoff to busa-messenger/busa-email) are always `handoff_ready` — this script never publishes a changelog, edits a roadmap doc, or sends a reply itself.
3. Show the operator the dry-run summary. After confirmation, run with `--apply`: LOCAL operations (roadmap lanes, merges) are applied directly to Busabase and the proposal's status is set `done`; outbound operations remain `handoff_ready` only.
4. Execute `handoff_ready` operations as the agent via the appropriate skill (send the decline reply through busa-messenger/busa-email, update the changelog/roadmap document), then log the outcome as a `sync-log` entry.
...
```bash
node skills/busa-feedback/scripts/ingest_feedback.mjs payload.json --apply
node skills/busa-feedback/scripts/apply_clusters.mjs assignments.json --apply
node skills/busa-feedback/scripts/execute_decisions.mjs
node skills/busa-feedback/scripts/execute_decisions.mjs --apply
pnpm --dir skills/busa-feedback/content/busa-feedback-app dev
```

In normal use, invoke `/busa-feedback`, let the skill ingest/cluster
