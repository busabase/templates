...
Use this schema when reading or writing Busa Brand's Busabase Bases. Field
slugs are kebab-case in Busabase and normalized to snake_case in app code
(`content/busa-brand-app/app/js/providers/busabase-provider.js`, `content/busa-brand-app/app/js/brand-model.js`).
...
## Drift Alerts (`busa-brand-drift-alerts`)
...
## Settings (`busa-brand-settings`)

One row per `kind`, looked up by `record-id`:

| `record-id` | `kind` | `payload` (JSON) |
| --- | --- | --- |
| `busa-brand-profile` | `profile` | `{brand: {name, category, audience, mission, framework}, style: {tone, reading_level, language}, official_urls: {homepage, about, ...}, risk_policy: {banned_phrases, regulated_claims}, channels: [{channel_id, type, display_name, monitored, secrets_ready}]}` |
| `busa-brand-lock` | `lock` | not JSON-wrapped: fields `locked` (bool), `owner`, `message` live directly on the row |
