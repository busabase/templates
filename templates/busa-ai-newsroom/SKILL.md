...
Use this skill as the operator's daily industry-intelligence operator for **AI/news-source intelligence**.
...
1. Read and follow `$kelly-app-skill-creator` for product behavior, visual quality, responsive layout, and the complete canonical `content/busa-ai-newsroom-app/` artifact.
...
Five Bases under one application Folder (`busa-ai-newsroom`), declared in
`content/busa-ai-newsroom-app/app/js/config.js` and the generated template sidecars under `content/`:
...
Start `pnpm --dir content/busa-ai-newsroom-app dev` only when local preview/debugging is explicitly
requested.
...
```bash
node skills/busa-ai-newsroom/scripts/execute_decisions.mjs
node skills/busa-ai-newsroom/scripts/execute_decisions.mjs --apply
pnpm --dir skills/busa-ai-newsroom/content/busa-ai-newsroom-app dev
```
