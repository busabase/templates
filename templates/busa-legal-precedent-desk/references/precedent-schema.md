...
in app code (`content/busa-legal-precedent-desk-app/app/js/providers/busabase-provider.js`,
`content/busa-legal-precedent-desk-app/app/js/precedent-model.js`). Metrics and the recent-activity feed are
...
`busa-legal-casebase-ingest`, `busa-legal-firm-radar`, and
`busa-legal-matter-strategy`, `revise` maps status back to `needs_review`
...
## Items (`busa-legal-precedent-desk-items`)
...
## Entities (`busa-legal-precedent-desk-entities`)
...
## Checks (`busa-legal-precedent-desk-checks`)
...
## Settings (`busa-legal-precedent-desk-settings`)
