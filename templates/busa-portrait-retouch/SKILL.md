...
metadata:
  ...
  busabase:
    template: true
    folderSlug: busa-portrait-retouch
    resources:
      - jobs
      - candidates
      - settings
...
The canonical App source is in `content/busa-portrait-retouch-app/`. It provides a quiet list/detail review
...
- Folder `busa-portrait-retouch` owns the workflow.
...
```bash
pnpm test
pnpm --dir content/busa-portrait-retouch-app check
