...
// version, just against Busabase records instead of content/busa-picks-app/.data/picks_snapshot.json.
//
...
import { appConfig } from "../content/busa-picks-app/app/js/config.js";
import { SOURCE_KINDS, STAGES } from "../content/busa-picks-app/app/js/picks-model.js";
...
async function create(client, declared, fields, message) {
  return client.bases.createChangeRequest({
    ...
    message,
    submittedBy: "busa-picks-ingest-trends",
    autoMerge: true,
  });
}
...
async function update(client, existing, fields, message) {
  return client.records.changeRequest({
    ...
    fields: toBusabaseFields(fields),
    message,
    author: "busa-picks-ingest-trends",
...
async function main() {
  ...
  if (!resources.folder || resources.missing.length) {
    throw new Error("Busa Picks Busabase resources are not provisioned yet; run the AirApp setup first.");
  }
  ...
  await create(
    client,
    declared("sync-log"),
    { log_id: `log-${Date.now().toString(36)}`, at: now, actor: "busa-picks-agent", action: "ingest_trends", detail },
    "Ingest sync log",
  );
