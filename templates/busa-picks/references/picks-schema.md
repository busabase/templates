...
Use this schema when reading or writing Busa Picks's Busabase Bases. Field
slugs are kebab-case in Busabase and normalized to snake_case in app code
(`content/busa-picks-app/app/js/providers/busabase-provider.js`, `content/busa-picks-app/app/js/picks-model.js`).
...
## Candidates (`busa-picks-candidates`)
...
Margin math (ported verbatim into `content/busa-picks-app/app/js/picks-model.js`'s `computeMarginCard()`, used by both `scripts/compute_margins.mjs` and the browser's live what-if panel):
...
## Trend Items (`busa-picks-trend-items`)
...
## Proposals (`busa-picks-proposals`)
