...
```bash
pnpm --dir skills/busa-family-office/content/busa-family-office-app dev
```
...
Four Bases under one application Folder (`busa-family-office`): `entities`, `accounts`, `holdings`, and `settings`. The AirApp is read-only — it only reads these Bases. See `SKILL.md` and `references/portfolio-schema.md` for exact field shapes.
