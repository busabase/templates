#!/usr/bin/env node
// Trusted hand-off step. Busa Family Office's AirApp is read-only (see
// content/busa-family-office-app/app/js/config.js — writeProcedures: []); this script is the only
...
import { appConfig } from "../content/busa-family-office-app/app/js/config.js";
...
async function main() {
  ...
  if (!resources.folder || resources.missing.length) {
    throw new Error("Busa Family Office Busabase resources are not provisioned yet; run the AirApp setup first.");
  }
  ...
  for (const fields of newEntities) {
    await client.bases.createChangeRequest({
      ...
      submittedBy: "busa-family-office-importer",
      autoMerge: true,
    });
  }
