...
<table>
  <tr>
    <td width="50%"><img src="assets/screenshots/overview.webp" alt="Busa Standup today board"></td>
    <td width="50%"><img src="assets/screenshots/blockers.webp" alt="Busa Standup blockers"></td>
  </tr>
  <tr>
  ...
  <tr>
    <td width="50%"><img src="assets/screenshots/members.webp" alt="Busa Standup members"></td>
    <td width="50%"><img src="assets/screenshots/reminders.webp" alt="Busa Standup reminders"></td>
  </tr>
  <tr>
...
Use this skill as the operator's team standup desk: see at a glance what everyone is
...
via busa-messenger / busa-email, after human approval.

## Mandatory Dependencies

1. Read and follow `$kelly-app-skill-creator` for product behavior, visual
   quality, responsive layout, and the complete canonical `content/busa-standup-app/` artifact.
...
- The skill may read team-consented channels only (busa-messenger snapshots, chat exports the user provides, shared docs, pasted text), parse them, and write the resulting structured updates and drafted reminders to Busabase via `scripts/ingest_updates.mjs`.
...
```bash
node skills/busa-standup/scripts/ingest_updates.mjs onboarding-payload.json --apply
```
...
Start `pnpm --dir content/busa-standup-app dev` only when local preview/debugging is explicitly
requested.
...
1. Ask where today's updates live (or use what the user already provided): the busa-messenger local snapshot, a Slack/WeCom/Discord/WhatsApp export, a shared doc, or text pasted into chat. Raw material stays outside git.
...
3. On "execute" / "send approved reminders": run `node scripts/execute_decisions.mjs --apply` to re-read approved reminders from Busabase and write a concrete `send_reminder` plan (channel, target, contact-env readiness, message draft) onto each — it performs no external side effect. Then send the actual messages via busa-messenger / busa-email using the `contact-env` referenced contacts, and record the outcome back onto the reminder (e.g. via a follow-up `ingest_updates.mjs` note, or by editing the record directly).
...
```bash
node skills/busa-standup/scripts/ingest_updates.mjs payload.json
node skills/busa-standup/scripts/ingest_updates.mjs payload.json --apply
node skills/busa-standup/scripts/execute_decisions.mjs
node skills/busa-standup/scripts/execute_decisions.mjs --apply
pnpm --dir skills/busa-standup/content/busa-standup-app dev
```

In normal use, invoke `/busa-standup`, let the skill ingest today's updates, and open the AirApp.
