...
// target changed, from content/busa-standup-app/.data/standup_snapshot.json to Busabase records.
//
...
import { appConfig } from "../content/busa-standup-app/app/js/config.js";
import {
  BLOCKER_STATUSES,
  MOODS,
  REMINDER_CHANNELS,
  ...
  SEVERITIES,
  UPDATE_SOURCES,
} from "../content/busa-standup-app/app/js/standup-model.js";
...
function fail(message) {
  console.error(`busa-standup ingest: ${message}`);
  process.exit(1);
}
...
async function upsertRow(client, declared, existing, idField, idValue, fields, message, apply) {
  ...
  await client.bases.createChangeRequest({
    ...
    fields: normalized,
    message,
    submittedBy: "busa-standup-ingest",
    autoMerge: true,
  });
  return "created";
}
...
async function main() {
  ...
  if (!resources.folder || resources.missing.length) {
    throw new Error("Busa Standup Busabase resources are not provisioned yet; run the AirApp setup first.");
  }
  ...
  for (const file of payloadFiles) {
    ...
    if (payload.team) {
      ...
      await upsertRow(
        ...
        "team",
        fields,
        "Update busa-standup team profile",
        apply,
      );
    ...
    for (const member of payload.members || []) {
      ...
      await upsertRow(
        ...
        member.member_id,
        fields,
        `Upsert busa-standup member ${member.member_id}`,
        apply,
      );
    ...
    for (const [index, item] of (payload.updates || []).entries()) {
      ...
      for (const [blockerIndex, blockerItem] of (item.blockers || []).entries()) {
        ...
        await upsertRow(
          ...
          blockerId,
          fields,
          `Upsert busa-standup blocker ${blockerId}`,
          apply,
        );
      ...
      await upsertRow(
        ...
        checkinId,
        fields,
        `Upsert busa-standup checkin ${checkinId}`,
        apply,
      );
      fileUpdates += 1;
    }
    ...
    for (const [index, item] of (payload.reminders || []).entries()) {
      ...
      await upsertRow(
        ...
        id,
        fields,
        `Upsert busa-standup reminder ${id}`,
        apply,
      );
