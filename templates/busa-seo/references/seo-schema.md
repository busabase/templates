...
Use this schema when reading or writing Busa SEO's Busabase Bases. Field
slugs are kebab-case in Busabase and normalized to snake_case in app code
(`content/busa-seo-app/app/js/providers/busabase-provider.js`, `content/busa-seo-app/app/js/seo-model.js`).
...
`badges` are computed by `badgesFor()` in `content/busa-seo-app/app/js/seo-model.js`:
...
## Opportunities (`busa-seo-opportunities`)
...
## GEO Opportunities (`busa-seo-geo-opportunities`)
...
`content/busa-seo-app/app/js/seo-model.js`) — recomputed live on every read from `draft` +
...
## AI Visibility (`busa-seo-ai-visibility`)
...
## Entity Signals (`busa-seo-entity-signals`)
...
`config.local.json` / `KELLY_SEO_CONFIG` / `~/.config/busa-seo/config.json`
...
`content/busa-seo-app/app/js/seo-model.js`):
