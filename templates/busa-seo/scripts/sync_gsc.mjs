#!/usr/bin/env node
// Read-only Google Search Console sync for Busa SEO. Ported from the retired
...
// settings Bases) instead of content/busa-seo-app/.data/seo_snapshot.json.
//
...
import { appConfig } from "../content/busa-seo-app/app/js/config.js";
import { badgesFor, ratio, round1 } from "../content/busa-seo-app/app/js/seo-model.js";
...
function fail(message) {
  console.error(`busa-seo sync: ${message}`);
  process.exit(1);
}
...
function configSearchPaths() {
  ...
  paths.push(path.join(process.env.HOME || "", ".config", "busa-seo", "config.json"));
  return paths;
}
...
function envSearchPaths() {
  ...
  paths.push(path.join(process.env.HOME || "", ".config", "busa-seo", ".env"));
  return paths;
}
...
async function resolveAccessToken(config) {
  ...
  fail(
    [
      ...
      "     (e.g. skills/busa-seo/.env.local) and add the service account email as a user on each",
...
async function main() {
  ...
  if (!configResult.path) {
    fail(
      "no private config found (config.local.json, KELLY_SEO_CONFIG, or ~/.config/busa-seo/config.json). Run onboarding first.",
    );
  }
  ...
  if (!resources.folder || resources.missing.length) {
    throw new Error("Busa SEO Busabase resources are not provisioned yet; run the AirApp setup first.");
  }
