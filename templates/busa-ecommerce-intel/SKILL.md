---
name: busa-ecommerce-intel
description: "Busa Ecommerce Intel: App-in-Skill daily industry intelligence cockpit for e-commerce and cross-border sellers (Busabase App-in-Skill). Use when the user asks about e-commerce, cross-border selling, product trends, competitor prices, listing optimization, ads,电商, or跨境 scenes. Prepares news/source signals, buyer-intent interpretation, approved sales actions, and channel drafts for review before any external handoff."
metadata:
  ...
  busabase:
    template: true
    folderSlug: busa-ecommerce-intel
    resources:
      - signals
      - actions
      - drafts
...
1. Read and follow `$kelly-app-skill-creator` for product behavior, visual quality, responsive layout, and the complete canonical `content/busa-ecommerce-intel-app/` artifact.
...
Five Bases under one application Folder (`busa-ecommerce-intel`), declared in
`content/busa-ecommerce-intel-app/app/js/config.js` and the generated template sidecars under `content/`:
...
Start `pnpm --dir content/busa-ecommerce-intel-app dev` only when local preview/debugging is explicitly
requested.
...
```bash
node skills/busa-ecommerce-intel/scripts/execute_decisions.mjs
node skills/busa-ecommerce-intel/scripts/execute_decisions.mjs --apply
pnpm --dir skills/busa-ecommerce-intel/content/busa-ecommerce-intel-app dev
```
