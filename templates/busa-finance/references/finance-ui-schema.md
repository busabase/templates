...
Use this schema when reading or writing Busa Finance's Busabase Bases.
Field slugs are kebab-case in Busabase and normalized to snake_case in app
code (`content/busa-finance-app/app/js/providers/busabase-provider.js`,
`content/busa-finance-app/app/js/finance-model.js`). The needs_review/approved/done/blocked
...
## Settings (`busa-finance-settings`)
...
`deriveModelMetrics(periods)` in `content/busa-finance-app/app/js/finance-model.js` computes
