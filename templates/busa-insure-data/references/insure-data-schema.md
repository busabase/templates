# Busa Insure Data UI Schema

This schema powers the AirApp UI for insurance data governance. It is the
`snapshot` object returned by `content/busa-insure-data-app/app/js/providers/busabase-provider.js`
(read from the operator-provisioned Busabase workspace) and
`content/busa-insure-data-app/app/js/providers/demo-provider.js` (deterministic offline fixture) —
...
```json
{
  ...
  "drive": {
    ...
    "metadata": {},
    "metadata_fields": [{ "key": "owner", "value": "You" }]
  },
  "bases": {
    "featured": {
      "base_id": "bse_...",
...
its home prompt rows. This AirApp does not declare it in `content/busa-insure-data-app/app/js/config.js`,
