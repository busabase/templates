// Trusted-process Busabase REST client for Busa Insure Data's skill-root
...
export function busabaseMeta({ envPrefix = "KELLY_INSURE_DATA" } = {}) {
  ...
  const noticesBaseSlug = "busa-insure-data-notices";
  return {
...
export function createBusabaseClient(options = {}) {
  ...
  function requireConfig() {
    if (!meta.baseUrl) throw new Error("Busa Insure Data Busabase client needs BUSABASE_BASE_URL.");
  }
  ...
  async function approveAndMerge(changeRequestId) {
    await api("POST", "/api/v1/change-requests/reviews", {
      ...
      verdict: "approved",
      reason: "Busa Insure Data Busabase operation requested with --apply.",
    });
  ...
  async function uploadAsset(localFile, mimeType, metadata = {}) {
    ...
    const upload = await api("POST", "/api/v1/assets/upload-urls", {
      ...
      context: "busa-insure-data/restore",
      contentHash,
    });
