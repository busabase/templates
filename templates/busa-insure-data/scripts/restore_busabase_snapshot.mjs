#!/usr/bin/env node
// Trusted operator script. Recreates a Busa Insure Data Busabase workspace
...
const manifestPath = String(
  args.manifest || path.join(skillDir, "content", "busa-insure-data-app", ".data", "busabase_restore_manifest.json"),
);
...
async function ensureFolderAndDrive(client, manifest) {
  ...
  await client.createNodeChangeRequest(operations, "Restore Busa Insure Data folder and Drive");
...
async function restoreFiles(client, driveNodeId, manifest) {
  ...
  for (const file of manifest.drive.files || []) {
    ...
    if (operations.length >= chunkSize) {
      const cr = await client.createDriveChangeRequest(
        driveNodeId,
        operations.splice(0),
        "Restore Busa Insure Data Drive files",
      );
  ...
  if (operations.length) {
    const cr = await client.createDriveChangeRequest(driveNodeId, operations, "Restore Busa Insure Data Drive files");
    await client.approveAndMerge(cr.id);
  }
  return {
...
async function restoreRecords(client, baseId, records, label) {
  ...
  for (let index = 0; index < missingRecords.length; index += chunkSize) {
    ...
    const cr = await client.bulkRecordChangeRequest(baseId, chunk, `Restore Busa Insure Data ${label} records`);
    await client.approveAndMerge(cr.id);
