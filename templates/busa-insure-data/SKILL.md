---
name: busa-insure-data
description: Insurance-industry App-in-Skill for read-only data governance, backed by an operator-provisioned Busabase workspace (one Drive node for the file drive plus four Bases for QA pairs, featured information, insurer notices, and user feedback) and trusted export/restore/PDF-text-backfill scripts. Use when the user invokes $busa-insure-data or /busa-insure-data, wants an insurance data workspace with UI, needs to review insurance files, metadata completeness, QA pairs, featured information, insurer notices, or user feedback, wants to back up or restore a Busa Insure Data Busabase workspace from local PDFs, or wants Busabase Drive/Base data surfaced for data quality review and ongoing data governance.
metadata:
...
1. Read and follow `$kelly-app-skill-creator` for product behavior, visual
   quality, responsive layout, and the complete canonical `content/busa-insure-data-app/` artifact.
...
owns the way most other Busa App-in-Skills do. There is nothing for a
read-only reader to safely auto-create in someone else's canonical dataset,
so:

- `content/busa-insure-data-app/app/js/config.js` declares the Drive node and the four Bases (by slug)
...
- `content/busa-insure-data-app/app/js/providers/busabase-provider.js` resolves the Drive node and
  each Base by slug (via `content/busa-insure-data-app/app/js/insure-client.js`, a raw-fetch client —
...
`content/busa-insure-data-app/app/js/insure-client.js` is therefore a small browser module that talks
straight to the same-origin `/api/v1/*` proxy in `content/busa-insure-data-app/server.js` with plain
...
<table>
  <tr>
    <td width="50%"><img src="assets/screenshots/overview.webp" alt="Busa Insure Data overview"></td>
    <td width="50%"><img src="assets/screenshots/files.webp" alt="Busa Insure Data file drive"></td>
  </tr>
  <tr>
  ...
  <tr>
    <td width="50%"><img src="assets/screenshots/qa.webp" alt="Busa Insure Data QA base"></td>
    <td width="50%"><img src="assets/screenshots/news.webp" alt="Busa Insure Data news base"></td>
  </tr>
  <tr>
...
Declared in `content/busa-insure-data-app/app/js/config.js` and the generated template sidecars under `content/`, resolved by
slug (IDs are never required):

- `drive` (`busa-insure-data-files`): the file drive — insurance PDFs/docs with
  governance metadata (`policy_type`, `carrier`, `region`, `effective_date`,
  `status`, ...).
- `featured` (`busa-insure-data-featured`) and `notices` (`busa-insure-data-notices`,
  legacy alias `news`): combined in the `#/news` route, each item tagged
  `featured` or `notice`.
- `qa` (`busa-insure-data-qa`): canonical insurance question/answer pairs.
- `feedback` (`busa-insure-data-feedback`): user feedback records.
- `prompts` (`insurance-prompts`, 预置提示词): the preset prompt library read by
  ...
  `content/busa-insure-data-app/app/js/config.js` and never appears in the snapshot, but it lives in the
...
```bash
cd skills/busa-insure-data
npm run busabase:export -- --output content/busa-insure-data-app/.data/busabase_restore_manifest.json
npm run busabase:restore -- --manifest content/busa-insure-data-app/.data/busabase_restore_manifest.json --files-root /path/to/local/pdf-backup --dry-run
...
Start `pnpm --dir content/busa-insure-data-app dev` only when local preview/debugging is explicitly
...
is **not** a deployment of this skill's `content/busa-insure-data-app/` artifact — it is an older
...
node as a thin viewer or replace its contents with this skill's `content/busa-insure-data-app/` artifact.
Keeping both means two divergent readers of one dataset; replacing means
uploading the full `content/busa-insure-data-app/` tree, including the vendored SDK, into the node.

## File Contract

Read `references/insure-data-schema.md` before editing the app or
`content/busa-insure-data-app/app/js/config.js`, and `references/restore-manifest-schema.md` before
...
- Keep real tokens in environment variables only; never commit
  ...
  `content/busa-insure-data-app/.data/`.
