#!/usr/bin/env node
// Trusted export step. Busa Legal Contracts's AirApp only ever writes a
...
// persisted content/busa-legal-contracts-app/.data/contract_snapshot.json + decisions.json to Busabase's
...
import { appConfig } from "../content/busa-legal-contracts-app/app/js/config.js";
import {
  normalizeContractRow,
  normalizeIssueRow,
} from "../content/busa-legal-contracts-app/app/js/contracts-model.js";
...
function issueMarkdown(issue, contract, sellerBrand) {
  ...
  lines.push(
    "---",
    "",
    `Exported by busa-legal-contracts on ${new Date().toISOString()} (Issue #${issue.ref}, ${issue.issue_id}). External sending or redline delivery is executed by the agent only after approval.`,
    "",
  );
  return lines.join("\n");
}
...
async function main() {
  ...
  if (!resources.folder || resources.missing.length) {
    throw new Error("Busa Legal Contracts Busabase resources are not provisioned yet; run the AirApp setup first.");
  }
  ...
  for (const row of exportableRows) {
    ...
    await client.records.changeRequest({
      ...
      author: "busa-legal-contracts-export",
