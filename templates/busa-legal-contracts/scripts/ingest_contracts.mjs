#!/usr/bin/env node
// Trusted hand-off step. Busa Legal Contracts's AirApp never ingests a
...
// from a persisted content/busa-legal-contracts-app/.data/contract_snapshot.json to Busabase's
...
async function upsert(client, declared, existing, fields, message, apply) {
  ...
  if (existing) {
    await client.records.changeRequest({
      ...
      fields: normalized,
      message,
      author: "busa-legal-contracts-ingest",
  ...
  await client.bases.createChangeRequest({
    ...
    fields: normalized,
    message,
    submittedBy: "busa-legal-contracts-ingest",
    autoMerge: true,
  });
  return "created";
}
