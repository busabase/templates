# Busa Legal Contracts Schema

Use this schema when reading or writing Busa Legal Contracts's Busabase
Bases. Field slugs are kebab-case in Busabase and normalized to snake_case in
app code (`content/busa-legal-contracts-app/app/js/providers/busabase-provider.js`,
`content/busa-legal-contracts-app/app/js/contracts-model.js`). Risk checks, per-issue risk scores,
...
## Contracts (`busa-legal-contracts-contracts`)
...
## Issues (`busa-legal-contracts-issues`)
...
## Checks (`busa-legal-contracts-checks`)
...
## Claims (`busa-legal-contracts-claims`)
...
## Claim Rules (`busa-legal-contracts-claim-rules`)
...
## Settings (`busa-legal-contracts-settings`)
...
the retired `content/busa-legal-contracts-app/server/rules.ts`:
