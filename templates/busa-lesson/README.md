...
Busa Lesson is a Busabase App-in-Skill desk for a dean of studies (教导主任) or training-program lead: the agent drafts lesson plans from curriculum materials and the school template, runs compliance checks against school quality standards, and the dean reviews, approves, and exports the plans through the App-in-Skill review queue.
...
Busa Lesson provisions its own Folder and four Bases (`teachers`, `plans`, `checks`, `settings`) lazily on first run in a Busabase Space — no manual setup required. See `SKILL.md`'s Busabase Resources section.
