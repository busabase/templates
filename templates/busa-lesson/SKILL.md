...
<table>
  <tr>
    <td width="50%"><img src="assets/screenshots/overview.webp" alt="Busa Lesson overview"></td>
    <td width="50%"><img src="assets/screenshots/needs-review.webp" alt="Busa Lesson review queue"></td>
  </tr>
  <tr>
  ...
  <tr>
    <td width="50%"><img src="assets/screenshots/checks.webp" alt="Busa Lesson compliance checks"></td>
    <td width="50%"><img src="assets/screenshots/plans.webp" alt="Busa Lesson plan library"></td>
  </tr>
  <tr>
...
1. Read and follow `$kelly-app-skill-creator` for product behavior, visual quality, responsive layout, and the complete canonical `content/busa-lesson-app/` artifact.
...
- Feedback to a teacher is approval-required: the app only stores a drafted note (`feedback-draft`) on the plan record; after the dean approves, the agent sends it via other channels/skills (for example busa-email) and records the real handoff separately. `scripts/execute_decisions.mjs` never performs this send itself — it only writes an execution marker.
...
Four Bases under one application Folder (`busa-lesson`), declared in `content/busa-lesson-app/app/js/config.js` and the generated template sidecars under `content/`:
...
Resources provision lazily through an idempotent Busabase ChangeRequest the first time the app runs in a Space; see `references/lesson-schema.md` for exact field shapes. Compliance scores, the review queue, the recent-activity feed, and metrics are recomputed client-side from the stored rows on every read (`content/busa-lesson-app/app/js/lesson-model.js`'s `buildSnapshot`/`assembleSnapshot`), so the desk is always fresh regardless of when a browser session loads it relative to the last ingest/checks run.
...
```bash
node skills/busa-lesson/scripts/ingest_plan.mjs payload.json --apply
node skills/busa-lesson/scripts/run_checks.mjs --apply
```

## Local App

Default behavior is AirApp-first — give the user the clickable AirApp URL. Start `pnpm --dir content/busa-lesson-app dev` only when local preview/debugging is explicitly requested.
...
1. Run `node skills/busa-lesson/scripts/run_checks.mjs --apply`. Deterministic rules (section presence, stage count and timing, duration sums, homework, measurable-verb heuristics, lab safety) are computed from the compliance rules on the Settings row; per-plan compliance scores are recomputed idempotently.
...
3. The agent then performs the approved follow-up outside the app: for `publish_plan`, run `scripts/export_plans.mjs` and send the feedback draft via other channels (e.g. busa-email) after approval; for `request_revision`, redraft the plan per the review note, re-ingest, and re-run checks.

## Export Workflow

1. `node skills/busa-lesson/scripts/export_plans.mjs --out <dir>` reads plans with status `approved` or `done` from Busabase and writes each as clean Markdown (default `exports/`, gitignored). Read-only against Busabase — no state is written back.
...
node skills/busa-lesson/scripts/ingest_plan.mjs payload.json --apply
node skills/busa-lesson/scripts/run_checks.mjs --apply
node skills/busa-lesson/scripts/execute_decisions.mjs
node skills/busa-lesson/scripts/execute_decisions.mjs --apply
node skills/busa-lesson/scripts/export_plans.mjs --out exports/
pnpm --dir skills/busa-lesson/content/busa-lesson-app dev
```

In normal use, invoke `/busa-lesson`, let the skill ingest/check what's due, and open the AirApp.
