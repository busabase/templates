#!/usr/bin/env node
// Trusted hand-off step. Busa Lesson's AirApp only ever proposes a review
...
// (content/busa-lesson-app/app/js/lesson-model.js's doc comment explains the adaptation: one
...
import { appConfig } from "../content/busa-lesson-app/app/js/config.js";
import { planExecution } from "../content/busa-lesson-app/app/js/lesson-model.js";
...
async function main() {
  ...
  if (!resources.folder || resources.missing.length) {
    throw new Error("Busa Lesson Busabase resources are not provisioned yet; run the AirApp setup first.");
  }
  ...
  for (const row of decided) {
    ...
    if (apply) {
      await client.records.changeRequest({
        ...
        author: "busa-lesson-execute-decisions",
