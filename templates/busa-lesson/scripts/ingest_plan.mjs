#!/usr/bin/env node
// Trusted hand-off step. Busa Lesson's AirApp never ingests a lesson plan
...
// only the write target changed, from a persisted content/busa-lesson-app/.data/lesson_snapshot.json
...
async function upsert(client, declared, existing, fields, message, apply) {
  ...
  await client.bases.createChangeRequest({
    ...
    fields: normalized,
    message,
    submittedBy: "busa-lesson-ingest",
    autoMerge: true,
  });
  return "created";
}
