#!/usr/bin/env node
// Trusted export step. Busa Lesson's AirApp only ever writes a review
...
// persisted content/busa-lesson-app/.data/lesson_snapshot.json to Busabase's teachers/plans
...
function planMarkdown(plan, teacher, schoolName) {
  ...
  lines.push(
    "---",
    "",
    `Exported by busa-lesson on ${new Date().toISOString()} (Plan #${plan.ref}, ${plan.plan_id}).`,
    "",
  );
  return lines.join("\n");
}
