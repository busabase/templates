#!/usr/bin/env node
// Trusted hand-off step for AI generation. Busa Drama's AirApp can only
...
import {
  ...
  voiceInstruct,
  voiceScript,
} from "../content/busa-drama-app/app/js/drama-model.js";
...
async function main() {
  ...
  for (const row of pendingVoices) {
    try {
      ...
      const { assetId } = await uploadAssetFromFile(client, outPath, "audio/wav", "busa-drama/generation");
