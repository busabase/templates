...
<table>
  <tr>
    <td width="50%"><img src="assets/screenshots/overview.webp" alt="Busa Drama overview"></td>
    <td width="50%"><img src="assets/screenshots/episodes.webp" alt="Busa Drama episode table"></td>
  </tr>
  <tr>
  ...
  <tr>
    <td width="50%"><img src="assets/screenshots/characters.webp" alt="Busa Drama character library"></td>
    <td width="50%"><img src="assets/screenshots/relationships.webp" alt="Busa Drama relationship map"></td>
  </tr>
  <tr>
...
Busa Drama is **not** the final motion editor. For final video tuning, each Busa Drama project should point to a concrete HyperFrame project path, and each episode should point to a concrete HyperFrame composition. Busa Drama manages the creative plan, canonical beats, storyboard metadata, review state, and asset index; HyperFrame owns the final composition, animation, captions, audio timing, render, and publish pass.

Default to the AirApp for ongoing creative work — give the user the clickable AirApp URL, or run `pnpm --dir content/busa-drama-app dev` for a local preview. Use chat-only mode only when the user explicitly asks for "chat only", "no UI", "纯聊天", or similar.

## Default Flow

1. Open the AirApp (or `pnpm --dir content/busa-drama-app dev` for local preview, which asks you to connect Busabase and select a Space — never an API key).
...
3. Use the app to maintain:
   ...
   - Episode HyperFrame links: each episode can carry `hyperframe_composition` (for example `index.html` or `compositions/ep-002.html`) and `hyperframe_video_asset` (the rendered/reference output indexed back into Busa Drama).
...
## Busa Drama ↔ HyperFrame Contract

Default architecture:

- One Busa Drama project = one HyperFrame project.
- One Busa Drama episode = one HyperFrame composition inside that project.
- Busa Drama is for planning and management; HyperFrame is for final visual/motion/audio polish.
...
3. Create/update the Busa Drama episode beats and storyboard shots to mirror the composition scenes.
...
When planning a new episode first in Busa Drama:

1. Draft the episode promise, beats, and storyboard shots in Busa Drama.
...
4. Treat the HyperFrame composition as the canonical final-cut surface once it exists; if the composition changes, re-import the new scene structure or update the Busa Drama shots to match.
...
One Folder (`busa-drama`), seven Bases, declared in `content/busa-drama-app/app/js/config.js` and the generated template sidecars under `content/`:
...
```bash
pnpm --dir skills/busa-drama/content/busa-drama-app dev
node skills/busa-drama/scripts/create_sample_project.mjs --apply
node skills/busa-drama/scripts/read_hyperframe_status.mjs --apply
node skills/busa-drama/scripts/validate_shot_readiness.mjs --episode ep-001
node skills/busa-drama/scripts/export_story_bible.mjs
node skills/busa-drama/scripts/execute_generation_requests.mjs --apply
```
...
Use paths relative to the skills repository root, or run the scripts from inside `skills/busa-drama`.
