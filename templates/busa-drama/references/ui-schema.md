...
Use this schema when reading or writing Busa Drama's Busabase Bases. Field
slugs are kebab-case in Busabase and normalized to snake_case in app code
(`content/busa-drama-app/app/js/providers/busabase-provider.js`). Completeness, attention
counts, and shot readiness are computed client-side from
`project`/`characters`/`relationships`/`episodes`/`shots`/`tasks` on every
read (`content/busa-drama-app/app/js/drama-model.js`) — they are never stored.
...
`content/busa-drama-app/server.js` and `content/busa-drama-app/app/js/drama-client.js` for the full trace). The
...
## characters (`busa-drama-characters`)
...
## relationships (`busa-drama-relationships`)
...
picks the active one (`content/busa-drama-app/app/js/providers/busabase-provider.js`'s
`setShotActive`).

## tasks (`busa-drama-tasks`)
