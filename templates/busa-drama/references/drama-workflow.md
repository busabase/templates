...
- One Busa Drama project maps to one explicit HyperFrame project path.
- One episode maps to one HyperFrame composition within that project.
- Busa Drama manages planning and review; HyperFrame is the final video workbench.
...
1. Run `node scripts/read_hyperframe_status.mjs --apply` to cache the HyperFrame project's current status onto the Busa Drama project record (the browser cannot read the local HyperFrame path directly).
...
4. Mirror each scene as a Busa Drama beat/shot, preserving source scene id and source time ranges.
...
For a new episode planned in Busa Drama first:

1. Plan beats and shots in Busa Drama.
...
4. Re-import rendered frames/video to Busa Drama when the composition becomes the reviewable or final version.
