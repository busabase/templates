...
metadata:
  ...
  busabase:
    template: true
    folderSlug: busa-sales-outreach
    resources:
      - profile
      - companies
      - leads
...
| --- | --- |
| `/busa-sales-outreach setup` | Create or adopt the Busabase Folder and three Bases, then verify them. |
| `/busa-sales-outreach profile` | Read product/service material, store the offer, derive an explicit ICP hypothesis, and optionally build a factual one-page PDF. |
| `/busa-sales-outreach research` | Discover and qualify target accounts and public business contacts, then draft one personalized first-touch email per company. |
| `/busa-sales-outreach send` | Configure the seller's mailbox in Vault and send only the individually approved queue. |
| `/busa-sales-outreach` | Open the desk and report the current bounded next step. |
...
1. Read `$kelly-app-skill-creator` for product behavior, responsive UI, and the canonical `content/busa-sales-outreach-app/` artifact.
...
- Keep the canonical Hono project in `<skill-root>/content/busa-sales-outreach-app/`; use the same source
...
| Resource | Purpose |
| --- | --- |
| `busa-sales-outreach-profile` | One offer, ICP hypothesis, proof, research channels, optional collateral, sender address, onboarding version, and Vault reference names. |
| `busa-sales-outreach-companies` | Target accounts with source evidence, pain signals, first-touch draft, review state, recipient, and send/opt-out timestamps. |
| `busa-sales-outreach-leads` | Public business contacts with company key, name/role, source URL, and confidence. |
