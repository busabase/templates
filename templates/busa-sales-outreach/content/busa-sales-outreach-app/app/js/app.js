...
const renderRows = (items, selectedId) => {
  if (!items.length) {
    return `<div class="empty-state"><p>这里还没有目标客户。</p><p class="empty-hint">运行研究命令，Agent 会推导 ICP 并把公司、业务信号和公开联系方式写回这里。</p>${commandChip("/busa-sales-outreach research")}</div>`;
  }
...
const renderCompanyDetail = (company) => {
  ...
  return `<div class="detail-scroll">
    ...
    ${
      company.bestLead
        ? ""
        : `<div class="detail-hint warn"><p class="detail-note warn">这家公司还没有可核验的业务邮箱，审批按钮暂时锁定。回到对话框补一次线索：</p>${commandChip("/busa-sales-outreach research", "补线索，不覆盖已批准和已触达记录")}</div>`
    }
...
const renderProfile = () => {
  ...
  return `<div class="profile-pane"><div class="detail-scroll">
    ...
    <div class="detail-hint">${commandChip("/busa-sales-outreach profile", "读取官网、产品说明或服务介绍，整理产品定位并提出 ICP")}</div>
    ...
    ${profile.mailReady ? "" : `<div class="detail-hint">${commandChip("/busa-sales-outreach send", "配置发件邮箱，授权码写进 Vault")}</div>`}
  ...
  <div class="modal-body">
    <section class="help-tab-panel ${helpTab === "commands" ? "active" : ""}"><h2>回到对话框能做什么</h2>
      ...
      <div class="command-list">
        ${commandChip("/busa-sales-outreach profile", "读取产品或服务资料，整理价值主张并提出 ICP")}
        ${commandChip("/busa-sales-outreach research", "从公开来源找目标客户、业务信号和联系人，每家写一封定制首触达")}
        ${commandChip("/busa-sales-outreach send", "配置发件邮箱，授权码写进 Vault，不进这个页面")}
