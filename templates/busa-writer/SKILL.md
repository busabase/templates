...
<table>
  <tr>
    <td width="50%"><img src="assets/screenshots/overview.webp" alt="Busa Writer overview"></td>
    <td width="50%"><img src="assets/screenshots/distribution.webp" alt="Busa Writer distribution review"></td>
  </tr>
  <tr>
...
Busa Writer is a Busabase Cloud App-in-Skill. Its canonical product surface
...
1. Read and follow `$kelly-app-skill-creator` for product behavior, visual
   quality, responsive layout, and the complete canonical `content/busa-writer-app/` artifact.
...
Two Bases under one application Folder (`busa-writer`), declared in
`content/busa-writer-app/app/js/config.js` and the generated template sidecars under `content/`:
...
- `settings`: one row per `kind` — an optional `busa-writer-profile` (brand,
  audience, official URLs, CTA defaults, channel defaults, risk terms, export
  preferences) and `busa-writer-lock`.
...
Start `pnpm --dir content/busa-writer-app dev` only when local preview/debugging is explicitly
requested.
