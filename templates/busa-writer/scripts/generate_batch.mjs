#!/usr/bin/env node
// Trusted ingestion step. Busa Writer's AirApp is review/decision-only (it
...
async function main() {
  ...
  const batchId = `busa-writer-${isoStamp()}`;
  ...
  if (!resources.folder || resources.missing.length) {
    throw new Error("Busa Writer Busabase resources are not provisioned yet; run the AirApp setup first.");
  }
  ...
  for (const item of items) {
    ...
    await client.bases.createChangeRequest({
      ...
      submittedBy: "busa-writer-generator",
      autoMerge: true,
    });
    created += 1;
  }
