...
Use this schema when reading or writing Busa CRM's Busabase Bases. Field
slugs are kebab-case in Busabase and normalized to snake_case in app code
(`content/busa-crm-app/app/js/crm-model.js`, `content/busa-crm-app/app/js/providers/busabase-provider.js`).
...
## Interactions (`busa-crm-interactions`)
...
## Settings (`busa-crm-settings`)

One row per `kind`, looked up by `record-id`:

| `record-id` | `kind` | `payload` (JSON) |
| --- | --- | --- |
| `busa-crm-operator` | `operator` | `{name, role, company, timezone, pipeline_stages, base_currency, style_tone}` |
| `busa-crm-channels` | `channels` | `{channels: [{channel_id, type, display_name, handoff_skill, vault_ref}]}` |
| `busa-crm-lock` | `lock` | not JSON-wrapped: fields `locked` (bool), `owner`, `message` live directly on the row |
...
Computed in `content/busa-crm-app/app/js/crm-model.js`, not stored in Busabase:
