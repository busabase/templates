...
import { appConfig } from "../content/busa-ppt-factory-app/app/js/config.js";
import {
  baseDeckFields,
  baseExportFields,
  normalizeDeckRow,
  ...
  normalizeSlideRow,
  normalizeStyleRow,
} from "../content/busa-ppt-factory-app/app/js/ppt-model.js";
...
function buildPptx({ deck, cards, brandName, style }) {
  ...
  pptx.author = "busa-ppt-factory";
  pptx.subject = deck.theme;
  pptx.title = deck.title;
  pptx.company = brandName || "You";
  pptx.theme = {
...
async function main() {
  ...
  if (!resources.folder || resources.missing.length) {
    throw new Error("Busa PPT Factory Busabase resources are not provisioned yet; run the AirApp setup first.");
  }
  ...
  const style = styleRows.length
    ...
    : null;
  const brandName = settings.brand_name || "You";
  ...
  await client.records.changeRequest({
    ...
    author: "busa-ppt-factory-generate-pptx",
  ...
  if (existingExport) {
    ...
    await client.bases.createChangeRequest({
      ...
      submittedBy: "busa-ppt-factory-generate-pptx",
      autoMerge: true,
    });
  }
