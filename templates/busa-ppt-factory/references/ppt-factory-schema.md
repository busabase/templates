# Busa PPT Factory Schema

Use this schema when reading or writing Busa PPT Factory's Busabase Bases.
Field slugs are kebab-case in Busabase and normalized to snake_case in app
code (`content/busa-ppt-factory-app/app/js/providers/busabase-provider.js`, `content/busa-ppt-factory-app/app/js/ppt-model.js`).
...
## Projects (`busa-ppt-factory-projects`)
...
## Slide Cards (`busa-ppt-factory-slideCards`)
...
## Style Systems (`busa-ppt-factory-styleSystems`)
...
`content/busa-ppt-factory-app/app/js/ppt-model.js`'s `defaultStyleSystem()`.

## QA Checks (`busa-ppt-factory-qaChecks`)
...
## Exports (`busa-ppt-factory-exports`)
...
## Settings (`busa-ppt-factory-settings`)
...
`content/busa-ppt-factory-app/app/js/ppt-model.js`'s `buildConfigSummary()` defaults.

## Review Queue (computed, never stored)

A deck or slide card is "in the review queue" when its own `review-summary`
is non-empty (`deriveReviewItems()` in `content/busa-ppt-factory-app/app/js/ppt-model.js`). The
