...
metadata:
  ...
  busabase:
    template: true
    folderSlug: busa-legal-matter-strategy
    resources:
      - items
      - entities
      - checks
...
1. Read and follow `$kelly-app-skill-creator` for product behavior, visual quality, responsive layout, and the complete canonical `content/busa-legal-matter-strategy-app/` artifact.
...
Four Bases under one application Folder (`busa-legal-matter-strategy`), declared in `content/busa-legal-matter-strategy-app/app/js/config.js` and the generated template sidecars under `content/`:
...
Resources provision lazily through an idempotent Busabase ChangeRequest the first time the app runs in a Space; see `references/strategy-schema.md` for exact field shapes. Metrics and the recent-activity feed are recomputed client-side from the stored rows on every read (`content/busa-legal-matter-strategy-app/app/js/matter-strategy-model.js`'s `buildSnapshot`/`assembleSnapshot`), so the desk is always fresh regardless of when a browser session loads it relative to the last ingest/decision.
...
```bash
node skills/busa-legal-matter-strategy/scripts/create_strategy_batch.mjs payload.json --apply
```

## Local App

Default behavior is AirApp-first — give the user the clickable AirApp URL. Start `pnpm --dir content/busa-legal-matter-strategy-app dev` only when local preview/debugging is explicitly requested.
...
```bash
node skills/busa-legal-matter-strategy/scripts/create_strategy_batch.mjs payload.json --apply
node skills/busa-legal-matter-strategy/scripts/execute_decisions.mjs
node skills/busa-legal-matter-strategy/scripts/execute_decisions.mjs --apply
node skills/busa-legal-matter-strategy/scripts/export_strategy_pack.mjs --out exports/
pnpm --dir skills/busa-legal-matter-strategy/content/busa-legal-matter-strategy-app dev
```

In normal use, invoke `/busa-legal-matter-strategy`, let the skill ingest what's due, and open the AirApp.
