#!/usr/bin/env node
// Idempotent: creates the busa-demo-video-factory Folder + videos/video-shots Bases only if missing.
...
// {appId, resourceKey, schemaVersion}` stamp matching content/busa-demo-video-factory-app/app/js/config.js's
...
// same convention every other Busabase-only the operator skill's browser-side provisioning
...
const APP_ID = "busa-demo-video-factory";
...
async function main() {
  ...
  if (!existingVideos) {
    const r = await createNodeChangeRequest(
      ...
      "Create busa-demo-video-factory folder + videos Base",
    );
