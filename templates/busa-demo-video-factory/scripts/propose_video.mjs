...
async function main() {
  ...
  let videoRecordId;
  if (shouldMerge) {
    const merged = await approveAndMerge(cfg, videoCr.id, "the operator approved via chat");
  ...
  for (const [i, shot] of outline.shots.entries()) {
    ...
    if (shouldMerge) {
      const merged = await approveAndMerge(cfg, shotCr.id, "the operator approved via chat");
  ...
  // content/busa-demo-video-factory-app/app/js/config.js). The AirApp's own read path never depends on this
  ...
  if (shouldMerge && videoRecordId && shotRecordIds.length > 0) {
    ...
    await approveAndMerge(cfg, current.id, "the operator approved via chat");
