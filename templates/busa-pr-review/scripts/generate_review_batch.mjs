...
import { appConfig } from "../content/busa-pr-review-app/app/js/config.js";
...
const USER_CONFIG_DIR = path.join(os.homedir(), ".config", "busa-pr-review");
...
function sampleItems() {
  ...
  return [
    ...
    },
    {
      ...
      labels: ["frontend"],
      changed_files: [
        "content/busa-pr-review-app/dashboard/EmptyState.tsx",
        "content/busa-pr-review-app/dashboard/styles.css",
      ],
      additions: 22,
      deletions: 5,
      comments_count: 0,
    ...
    },
    {
      ...
      risk: ["frontend"],
      labels: ["frontend"],
      changed_files: ["content/busa-pr-review-app/profile/Settings.tsx"],
      additions: 48,
      deletions: 9,
      comments_count: 1,
      state: "closed",
...
async function main() {
  ...
  if (!resources.folder || resources.missing.length) {
    throw new Error("Busa PR Review Busabase resources are not provisioned yet; run the AirApp setup first.");
  }
  ...
  for (const item of items) {
    ...
    if (existing) {
      await client.records.changeRequest({
        ...
        author: "busa-pr-review-generator",
      ...
      await client.bases.createChangeRequest({
        ...
        submittedBy: "busa-pr-review-generator",
        autoMerge: true,
      });
      created += 1;
    }
  ...
  const existingProfile = await findByFieldSlug(client, settingsBase.baseId, "record-id", "busa-pr-review-profile");
  ...
  const profileFields = toBusabaseFields({
    record_id: "busa-pr-review-profile",
    kind: "busa-pr-review-profile",
  ...
  if (existingProfile) {
    await client.records.changeRequest({
      ...
      message: "Refresh Busa PR Review profile",
      author: "busa-pr-review-generator",
    ...
    await client.bases.createChangeRequest({
      ...
      fields: profileFields,
      message: "Write Busa PR Review profile",
      submittedBy: "busa-pr-review-generator",
      autoMerge: true,
    });
  }
