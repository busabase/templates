#!/usr/bin/env node
// Trusted execution step. Busa PR Review's AirApp only ever writes a human
...
async function submitReview(fields, apply) {
  ...
  const tempPath = path.join(
    os.tmpdir(),
    `busa-pr-review-${String(fields.repo).replaceAll("/", "-")}-${fields.number}-${Date.now()}.txt`,
  );
  await fs.writeFile(tempPath, body || "Reviewed from Busa PR Review.", "utf8");
  try {
    const args = [
      "pr",
      "review",
  ...
  for (const record of records) {
    ...
    if (apply && (outcome.status === "executed" || outcome.status === "skipped")) {
      await client.records.changeRequest({
        ...
        author: "busa-pr-review-executor",
