# Busa PR Review Schema

Use this schema when reading or writing Busa PR Review's Busabase Bases.
Field slugs are kebab-case in Busabase and normalized to snake_case in app
code (`content/busa-pr-review-app/app/js/providers/busabase-provider.js`,
`content/busa-pr-review-app/app/js/pr-review-model.js`). Workflow-status bucketing, decision-status
...
## Reviews (`busa-pr-review-reviews`)
...
## Settings (`busa-pr-review-settings`)

One row per `kind`, looked up by `record-id`:

| `record-id` | `kind` | Shape |
| --- | --- | --- |
| `busa-pr-review-profile` | `profile` | `payload` (JSON): `{reviewer: {handle, display_name}, repos: [{repo, label, include}], query: {state, review_requested, limit, merged_limit, merged_at, sort, order, include_drafts}, review_policy: {default_action, include_patch_excerpt, max_patch_chars, large_diff_changed_files, large_diff_additions, risk_keywords}, style: {tone}}` |
| `busa-pr-review-lock` | `lock` | `payload` (JSON): `{locked, message, owner, started_at}` |
...
`~/.config/busa-pr-review/config.json` → gh defaults). The AirApp only ever
...
`countByWorkflow()` in `content/busa-pr-review-app/app/js/pr-review-model.js`.
...
`reviews` (keyed by `item-id`) plus the `busa-pr-review-profile` settings
