---
name: busa-pr-review
description: GitHub pull request review desk (Busabase App-in-Skill) using the gh CLI for real GitHub access, an AirApp review UI for human approval, and a trusted gh pr review execution step. Use when the user invokes /busa-pr-review or $busa-pr-review, asks to review GitHub PRs, generate a PR review batch, approve/comment/request changes through the review UI, or execute approved GitHub reviews.
metadata:
  category: platform
  tags:
    - risk:gated-write
...
<table>
  <tr>
    <td width="50%"><img src="assets/screenshots/overview.webp" alt="Busa PR Review overview"></td>
    <td width="50%"><img src="assets/screenshots/needs-review.webp" alt="Busa PR Review needs review"></td>
  </tr>
  <tr>
  ...
  <tr>
    <td width="50%"><img src="assets/screenshots/ready.webp" alt="Busa PR Review ready to approve"></td>
    <td width="50%"><img src="assets/screenshots/blocked-security.webp" alt="Busa PR Review blocked review"></td>
  </tr>
  <tr>
  ...
  <tr>
    <td width="50%"><img src="assets/screenshots/needs-test.webp" alt="Busa PR Review merged PR needs test"></td>
    <td width="50%"><img src="assets/screenshots/tested.webp" alt="Busa PR Review tested verification"></td>
  </tr>
  <tr>
...
1. Read and follow `$kelly-app-skill-creator` for product behavior, visual
   quality, responsive layout, and the complete canonical `content/busa-pr-review-app/` artifact.
...
2. `skills/busa-pr-review/config.local.json`
3. `~/.config/busa-pr-review/config.json`
...
4. `skills/busa-pr-review/.env.local`
5. `~/.config/busa-pr-review/.env`
...
written to Busabase's `busa-pr-review-profile` settings row (see
`references/ui-schema.md`) so the AirApp can display it — the AirApp never
writes that row itself.

## Busabase Resources

Two Bases under one application Folder (`busa-pr-review`), declared in
`content/busa-pr-review-app/app/js/config.js` and the generated template sidecars under `content/`:
...
- `settings`: one row per `kind` — `busa-pr-review-profile` (reviewer,
  repos, query, review policy, style, written only by
  `scripts/generate_review_batch.mjs --apply`) and `busa-pr-review-lock`.
...
```bash
node skills/busa-pr-review/scripts/generate_review_batch.mjs --apply
node skills/busa-pr-review/scripts/generate_review_batch.mjs --sample
node skills/busa-pr-review/scripts/execute_decisions.mjs
node skills/busa-pr-review/scripts/execute_decisions.mjs --apply
pnpm --dir skills/busa-pr-review/content/busa-pr-review-app dev
```

In normal use, invoke `/busa-pr-review`, let the skill generate/refresh the
