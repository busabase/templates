...
const CONFIG_RECORD_ID = "busa-email-config";
const LOCK_RECORD_ID = "busa-email-lock";
const SCAN_STATE_RECORD_ID = "busa-email-scan-state";
const DEFAULT_LOCK_MESSAGE = "/busa-email is processing this batch.";
...
export function createBusabaseProvider() {
  ...
  async function readConfigMeta(): Promise<ConfigWithMeta> {
    ...
    return {
      config,
      source: row ? "busabase:base/busa-email-settings-v3" : "",
      reader: "busabase",
      provider: "busabase",
      is_example: false,
      ...
      recommended_config: "busabase:base/busa-email-settings-v3",
  ...
  const provider = {
    ...
    onboardingStatus(config: Config, meta: ConfigWithMeta = {} as ConfigWithMeta) {
      ...
      return {
        ...
        message: !hasConfig
          ? "Busa Email resources are ready. Add an account configuration record to Email Settings."
          : missing.length
    ...
    async getLock() {
      ...
      return row.locked
        ? {
            ...
            owner: row.owner || "busa-email-agent",
    ...
    async writeLock(message: string) {
      ...
      await writeSettings(
        LOCK_RECORD_ID,
        {
          ...
          name: "Agent Lock",
          locked: true,
          owner: "busa-email-agent",
          message,
          started_at: utcNow(),
          updated_at: utcNow(),
        },
        "Set Busa Email agent lock",
      );
    },
    ...
    async init() {
      ...
      try {
        ...
        return {
          ...
          message: "Busa Email is connected to Busabase Cloud.",
          action: "",
        };
      } catch (error) {
        return {
          ...
          base_url: busabase.meta.baseUrl || "same-origin",
          space_id: busabase.meta.spaceId,
          message: "Busa Email could not initialize its declared Busabase resources.",
