...
function accountsCardsHtml() {
  ...
  const promptCard = `
    <article class="account-help-card">
      ...
      <pre><code>/busa-email 帮我增加一个 email account：邮箱是 name@example.com，IMAP/SMTP 是 example.com，alias 有 support@example.com，用 Support 身份回复。请更新当前 provider config，但不要让我在聊天里贴密码。

/busa-email 给 main 账号增加 alias：hello@example.com，并新增一个 outbound identity：display name 是 Founder，send_as 是 founder@example.com。

/busa-email 测试当前 email account 配置，告诉我缺哪些 secret ref 或 Vault secret。</code></pre>
    </article>
  `;
