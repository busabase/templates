...
const localGateShell = (title, body, footer = "") => `
  <section class="provider-gate-panel local-connect-panel" aria-labelledby="localSetupTitle">
    <div class="provider-gate-head"><div class="provider-gate-kicker">Busa Email</div></div>
...
export function setupPrompt() {
  ...
  if (status.ok === false) {
    return "Open Busa Email in the selected Busabase Space and initialize the declared resources.";
  }
  if (missing.length) {
    return `Add the referenced mailbox secrets to Busabase Vault namespace busa-email: ${missing.join(", ")}.`;
  }
  return "Open the Busa Email Settings Base and add at least one mailbox configuration record.";
}
...
export function setupChecklistHtml() {
  ...
  const rows = [
    ...
    [t("setup.check.folder"), Boolean(connection.folder_exists), status.folder_slug || "busa-email"],
    [t("setup.check.base"), Boolean(connection.base_exists), status.base_id || "busa-email-reviews-v3"],
    [
      t("setup.check.contacts"),
      Boolean(connection.contacts_base_exists),
      status.contacts_base_id || "busa-email-contacts-v3",
    ],
    ["Settings Base", Boolean(connection.settings_base_exists), status.settings_base_id || "busa-email-settings-v3"],
    [t("setup.check.drive"), Boolean(connection.drive_exists), status.drive_slug || "busa-email-files-v3"],
    [
      t("setup.check.config"),
      Boolean(onboarding.has_config),
      setup.recommended_config || "busabase:base/busa-email-settings-v3",
    ],
    [
      t("setup.check.secrets"),
      Boolean(onboarding.has_config && missingSecrets.length === 0),
      missingSecrets.length ? missingSecrets.join(", ") : setup.recommended_env || "busabase:vault/busa-email",
    ],
  ];
  return rows
    .map(
