...
export async function statePayload(query: StateQuery = {}) {
  ...
  const onboarding = providerUnavailable
    ? {
        ...
        message: providerState.message || "Busa Email data provider is not ready.",
        missing_env: [],
      }
  ...
  const recommendedConfig =
    onboarding.recommended_config || configMeta.recommended_config || "busabase:base/busa-email-settings-v3";
  const recommendedEnv =
    onboarding.recommended_env ||
    configMeta.recommended_env ||
    `busabase:vault/${String(providerState.secrets_namespace || "busa-email")}`;
  return {
    app: "busa-email",
    setup: {
    ...
    contacts_path: "busabase:base/busa-email-contacts-v3",
    decisions_path: "busabase:base/busa-email-reviews-v3/decision-*",
