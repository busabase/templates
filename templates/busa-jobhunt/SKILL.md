---
name: busa-jobhunt
description: Run a Busabase-backed job-search outreach desk with a bundled Hono App-in-Skill deployable to AirApp. Use when the user invokes $busa-jobhunt or /busa-jobhunt, wants to turn a resume into a structured profile and a typeset PDF, wants to find target companies and their contact addresses instead of applying through a job board, wants one tailored application email drafted per company, or wants to review and approve each send. Four subcommands - setup, profile, research, send - plus the desk itself. One company receives exactly one email; mailbox credentials live in the Busabase Vault and never reach browser code.
metadata:
  category: comms
  tags:
    - risk:gated-write
...
| --- | --- |
| `/busa-jobhunt setup` | Create or adopt the Busabase Folder and three Bases, then read them back |
| `/busa-jobhunt profile` | Take the user's resume or raw notes, extract a structured profile, ask which job channels to search, and build a typeset PDF resume |
| `/busa-jobhunt research` | Find target companies and their contact addresses, draft one email each, import them |
| `/busa-jobhunt send` | Put the user's own SMTP credentials in the Busabase Vault, rehearse the transport, then send what they approved |
| `/busa-jobhunt` (no argument) | Open the desk and report where things stand |
...
2. initialize schema v4 resources, then show product onboarding v1 with
   `/busa-jobhunt profile` as the primary path. The user may skip it and enter
...
1. Read and follow `$kelly-app-skill-creator` for product behavior, visual quality, responsive layout, and the complete local `content/busa-jobhunt-app/` artifact.
...
5. Read `references/research-playbook.md` before running `/busa-jobhunt research` for the first time — it is the working method behind the boundaries stated here.
...
- **Research** — `/busa-jobhunt research` collects companies and addresses with
...
- Keep the complete canonical project under `<skill-root>/content/busa-jobhunt-app/` and provide a
...
| Resource | Purpose |
| --- | --- |
| `busa-jobhunt-profile` | One row. Identity, target role, channels, resume source text, resume file name, sender address, onboarding version, and the SMTP Vault reference names. |
| `busa-jobhunt-companies` | One row per company: match evidence, drafted email, outreach status, the address actually used. |
| `busa-jobhunt-leads` | Several rows per company: candidate addresses with role, source URL, and confidence. |
