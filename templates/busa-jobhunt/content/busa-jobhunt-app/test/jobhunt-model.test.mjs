...
test("the desk names the next subcommand for every state it can be in", () => {
  ...
  assert.equal(nextStep(empty).command, "/busa-jobhunt profile");

  // Profile ready but no companies yet.
  const readyProfile = createJobhuntDesk([profileRecord]);
  assert.equal(nextStep(readyProfile).command, "/busa-jobhunt research");

  // A company with no address sends you back to research rather than letting
  // you stare at a disabled button.
  assert.equal(nextStep(createJobhuntDesk(sample)).command, "/busa-jobhunt research");
  ...
  assert.equal(nextStep(createJobhuntDesk(queued)).command, "/busa-jobhunt send");
