...
Authoritative field slugs, status values, and Vault keys. `content/busa-jobhunt-app/app/js/config.js`
...
outreach should be sent without an attachment, and `/busa-jobhunt profile` may
...
continues to name `/busa-jobhunt profile` as the next step.

## `busa-jobhunt-companies` — 目标公司
...
| --- | --- | --- | --- |
| `name` | text | yes | Primary field. |
| `key` | text | yes | Stable domain key; `busa-jobhunt-leads.company-key` points at it. |
| `website` | text | no | |
...
| `company-key` | text | yes | Plain text foreign key into `busa-jobhunt-companies.key`. |
