...
// the Agent during `/busa-jobhunt profile`. Blank lines separate blocks; a
...
if (!row) fail("求职档案还没填。先跑 /busa-jobhunt profile，把你的简历交给它。");
...
if (!profile.name || !profile.targetRole) fail("求职档案缺少姓名或目标岗位，排不出简历。先跑 /busa-jobhunt profile。");
if (!profile.resumeSource && !profile.highlights) {
  fail("求职档案里没有任何简历内容（简历原文与自我介绍都是空的）。先跑 /busa-jobhunt profile。");
}
...
console.log("下一步：/busa-jobhunt research  去找目标公司。");
