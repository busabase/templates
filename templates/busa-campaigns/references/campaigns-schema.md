# Busa Campaigns Schema

Use this schema when reading or writing Busa Campaigns' Busabase Bases.
Field slugs are kebab-case in Busabase and normalized to snake_case in app
code (`content/busa-campaigns-app/app/js/providers/busabase-provider.js`,
`content/busa-campaigns-app/app/js/campaigns-model.js`). Metrics, the pre-send deliverability-risk
...
## Segments (`busa-campaigns-segments`)
...
- `deliverability.risk` is derived, never stored: `high` when auth fails,
  ...
  `deliverabilityInfo()` in `content/busa-campaigns-app/app/js/campaigns-model.js`.

## Suppression (`busa-campaigns-suppression`)
...
`checkSuppression()` in `content/busa-campaigns-app/app/js/campaigns-model.js`.

## Settings (`busa-campaigns-settings`)

One row per `kind`, looked up by `record-id`:

| `record-id` | `kind` | Shape |
| --- | --- | --- |
| `busa-campaigns-profile` | `profile` | `payload` (JSON): `{operator: {name, role, company, timezone}, brand: {name, homepage, unsubscribe_url}, esp: {provider, display_name, secrets_ready}, from_identities: [{identity_id, from_name, from_email, reply_to, use_when}], sending_policy: {approval_required, daily_send_cap, hourly_send_cap, min_inbox_readiness, max_spam_score}, style_tone, list_health: {subscriber_count, bounce_rate, complaint_rate, churn_rate, avg_open_rate, avg_click_rate}}` |
| `busa-campaigns-lock` | `lock` | not JSON-wrapped: fields `locked` (bool), `owner`, `message` live directly on the row |
