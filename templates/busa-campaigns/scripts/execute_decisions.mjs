#!/usr/bin/env node
// Trusted hand-off step. Busa Campaigns' AirApp only ever proposes a review
...
import { buildSnapshot, checkSuppression } from "../content/busa-campaigns-app/app/js/campaigns-model.js";
import { appConfig } from "../content/busa-campaigns-app/app/js/config.js";
...
async function main() {
  ...
  if (!resources.folder || resources.missing.length) {
    throw new Error("Busa Campaigns Busabase resources are not provisioned yet; run the AirApp setup first.");
  }
    ...
    if (apply) {
      ...
      if (record) {
        ...
        await client.records.changeRequest({
          ...
          author: "busa-campaigns-executor",
  ...
  console.log(JSON.stringify({ executed_at: now, dry_run: !apply, source: "busa-campaigns", esp, results }, null, 2));
