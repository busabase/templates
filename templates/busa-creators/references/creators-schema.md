# Busa Creators Schema

Use this schema when reading or writing Busa Creators' Busabase Bases.
Field slugs are kebab-case in Busabase and normalized to snake_case in app
code (`content/busa-creators-app/app/js/providers/busabase-provider.js`,
`content/busa-creators-app/app/js/creators-model.js`). `phase`, `cpm`, and every rollup metric are
...
## Creators (`busa-creators-creators`)
...
## Settings (`busa-creators-settings`)

One row per `kind`, looked up by `record-id`:

| `record-id` | `kind` | `payload` (JSON) |
| --- | --- | --- |
| `busa-creators-profile` | `profile` | `{operator: {name, role, company, timezone}, program: {base_currency, budget_total, target_niches}, brands: [{brand_id, display_name, positioning}], style: {tone}, platforms: [{platform_id, type, display_name, handoff_skill, secret_envs, secrets_ready}]}` |
| `busa-creators-lock` | `lock` | not JSON-wrapped: fields `locked` (bool), `owner`, `message` live directly on the row |
