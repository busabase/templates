#!/usr/bin/env node
// Trusted hand-off step. Busa Audit's AirApp never imports business
...
// persisted content/busa-audit-app/.data/audit_snapshot.json to Busabase's orders/invoices/
...
// content/busa-audit-app/app/js/audit-model.js's deriveSnapshot(), so this importer only needs
...
import { appConfig } from "../content/busa-audit-app/app/js/config.js";
...
function fail(message) {
  console.error(`busa-audit import: ${message}`);
  process.exitCode = 1;
...
async function main() {
  ...
  if (!resources.folder || resources.missing.length) {
    throw new Error("Busa Audit Busabase resources are not provisioned yet; run the AirApp setup first.");
  }
