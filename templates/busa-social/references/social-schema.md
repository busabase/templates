...
Use this schema when reading or writing Busa Social's Busabase Bases. Field
slugs are kebab-case in Busabase and normalized to snake_case in app code
(`content/busa-social-app/app/js/providers/busabase-provider.js`, `content/busa-social-app/app/js/social-model.js`).
...
channels})` in `content/busa-social-app/app/js/social-model.js` recomputes the social-qa
...
## Engagement (`busa-social-engagement`)
...
## Settings (`busa-social-settings`)

One row per `kind`, looked up by `record-id`:

| `record-id` | `kind` | `payload` (JSON) |
| --- | --- | --- |
| `busa-social-crisis` | `crisis` | `{status: "calm\|watch\|active", publishing_paused, spokesperson?, updated_at?, steps: [{step_id, label, detail, owner?, done}]}` |
| `busa-social-share-of-voice` | `share_of_voice` | `{window: "7d", total_mentions, entries: [{name, is_self, mentions_7d, share}]}` (`share` is a 0-1 fraction; exactly one entry should have `is_self: true`) |
...
- `crisis_toggle`: read-modify-writes the `busa-social-crisis` settings row
