...
// only the storage target changed, from content/busa-social-app/.data/social_snapshot.json to
...
import { appConfig } from "../content/busa-social-app/app/js/config.js";
...
function fail(message) {
  console.error(`busa-social ingest: ${message}`);
  process.exit(1);
}
...
async function upsertRow(client, declared, existing, fields, message, apply) {
  ...
  await client.bases.createChangeRequest({
    ...
    fields: normalized,
    message,
    submittedBy: "busa-social-ingest",
    autoMerge: true,
  });
  return "created";
}
...
async function main() {
  ...
  if (!resources.folder || resources.missing.length) {
    throw new Error("Busa Social Busabase resources are not provisioned yet; run the AirApp setup first.");
  }
  ...
  for (const incoming of payload.accounts) {
    ...
    await upsertRow(
      ...
      null,
      {
        ...
        actor: payload.source || "busa-social",
      },
