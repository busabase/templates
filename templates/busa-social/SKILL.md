...
name: busa-social
description: >-
  Social media command desk (Busabase App-in-Skill) for the operator's Twitter/X,
  ...
  publishing. Use when the user invokes $busa-social or /busa-social, wants
...
Busa Social is a Busabase Cloud App-in-Skill. Its canonical product surface
...
bootstrap. Use this skill as the operator's social media command desk. It does two
...
1. Read and follow `$kelly-app-skill-creator` for product behavior, visual
   quality, responsive layout, and the complete canonical `content/busa-social-app/` artifact.
...
- The skill must never create fake engagement: no automated likes, follows,
  comments, reposts, or engagement pods. Normal Busa Social operation is
...
Eight Bases under one application Folder (`busa-social`), declared in
`content/busa-social-app/app/js/config.js` and the generated template sidecars under `content/`:
...
- `drafts`: the post composer / draft review queue. The social-qa quality
  ...
  `content/busa-social-app/app/js/social-model.js`'s `evaluateGate()`) — it is never trusted as
...
- `engagement`: incoming mentions/comments with an agent-drafted reply,
  approval-gated.
- `settings`: one row per `kind` — `busa-social-crisis` (the crisis
  playbook) and `busa-social-share-of-voice`.
...
Start `pnpm --dir content/busa-social-app dev` only when local preview/debugging is explicitly
requested.
...
The quality gate (⛩ `social-qa`, in `content/busa-social-app/app/js/social-model.js`): every draft is scored 0–100 (SQS) across brand voice, disclosure, and banned claims, producing a **SHIP / FIX / BLOCK** verdict. A BLOCK forces the draft to `blocked` and disables approve/publish until it is revised.
...
`busa-social-crisis` settings row. From a standalone local preview the write
...
6. Give the operator the AirApp URL (or local preview URL) to review the fresh dashboard.
