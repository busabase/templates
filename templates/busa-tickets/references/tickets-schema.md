...
Use this schema when reading or writing Busa Tickets' Busabase Bases. Field
slugs are kebab-case in Busabase and normalized to snake_case in app code
(`content/busa-tickets-app/app/js/providers/busabase-provider.js`, `content/busa-tickets-app/app/js/tickets-model.js`).
...
## Dispatch Proposals (`busa-tickets-proposals`)
...
## Sync Log (`busa-tickets-sync-log`)
...
| `source` | `source` | text | `wechat_export`\|`phone_log`\|`form`\|`email`\|`busa-tickets` |
| `action` | `action` | text | `ingest`\|`triage`\|`execute` |
| `detail` | `detail` | longtext | human-readable summary |
| `count` | `count` | number | |

## Settings (`busa-tickets-settings`)
