...
Contacts are masked before they reach Busabase. Classification and dispatch proposals merge through `node skills/busa-tickets/scripts/apply_triage.mjs payload.json --apply` (SLA targets computed from the Settings row's `sla_rules`); approved dispatches become a concrete plan via `node skills/busa-tickets/scripts/execute_decisions.mjs --apply` (dry-run by default). All three scripts are dry runs by default. See `references/tickets-schema.md`.
