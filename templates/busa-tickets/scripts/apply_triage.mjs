...
// load are never written — content/busa-tickets-app/app/js/tickets-model.js's buildSnapshot()
...
// persisted content/busa-tickets-app/.data/tickets_snapshot.json to Busabase's intake/tickets/
...
import { appConfig } from "../content/busa-tickets-app/app/js/config.js";
import { slaHoursFor } from "../content/busa-tickets-app/app/js/tickets-model.js";
...
function fail(message) {
  console.error(`busa-tickets triage: ${message}`);
  process.exitCode = 1;
...
async function createRow(client, declared, fields, message, apply) {
  ...
  await client.bases.createChangeRequest({
    ...
    message,
    submittedBy: "busa-tickets-triage",
    autoMerge: true,
  });
}
...
async function main() {
  ...
  if (!resources.folder || resources.missing.length) {
    throw new Error("Busa Tickets Busabase resources are not provisioned yet; run the AirApp setup first.");
  }
  ...
  for (const entry of classifications) {
    ...
    const history = [
      { event: "intake", actor: "busa-tickets", at: item.received_at, note: `Received via ${item.channel}.` },
      {
        event: "classified",
        actor: "busa-tickets",
        at: now,
  ...
  for (const entry of ticketUpdates) {
    ...
    const history = parseJsonValue(ticket.history, []) || [];
    history.push({ event, actor: String(entry.actor || "busa-tickets"), at: now, note: String(entry.note || "") });
