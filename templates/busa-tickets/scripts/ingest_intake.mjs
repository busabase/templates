#!/usr/bin/env node
// Trusted hand-off step. Busa Tickets' AirApp never imports complaint
...
// persisted content/busa-tickets-app/.data/tickets_snapshot.json to Busabase's intake/sync_log
// Bases.
//
// Usage:
...
import { maskContact } from "../content/busa-tickets-app/app/js/tickets-model.js";
...
function fail(message) {
  console.error(`busa-tickets ingest: ${message}`);
  process.exitCode = 1;
    ...
    for (const [index, item] of payload.items.entries()) {
      ...
      if (apply) {
        await client.bases.createChangeRequest({
          ...
          submittedBy: "busa-tickets-ingest",
          autoMerge: true,
        });
      }
      fileAdded += 1;
