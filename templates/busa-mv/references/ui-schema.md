...
Use this schema when reading or writing Busa MV's Busabase Bases. Field
slugs are kebab-case in Busabase and normalized to snake_case in app code
(`content/busa-mv-app/app/js/providers/busabase-provider.js`). Checklist completeness,
"next step" guidance, and status counts are computed client-side from
`project`/`cast`/`shots` on every read (`content/busa-mv-app/app/js/mv-model.js`) — they are
never stored.
...
`content/busa-mv-app/server.js` and `content/busa-mv-app/app/js/mv-client.js` for the full trace). The code
...
(`content/busa-mv-app/app/js/providers/busabase-provider.js`'s `setShotActive`).
