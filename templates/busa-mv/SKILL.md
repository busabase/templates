...
<table>
  <tr>
    <td width="50%"><img src="assets/screenshots/overview.webp" alt="Busa MV concept view"></td>
    <td width="50%"><img src="assets/screenshots/storyboard.webp" alt="Busa MV storyboard"></td>
  </tr>
  <tr>
  ...
  <tr>
    <td width="50%"><img src="assets/screenshots/cast.webp" alt="Busa MV cast"></td>
    <td width="50%"><img src="assets/screenshots/song.webp" alt="Busa MV song"></td>
  </tr>
  <tr>
...
An MV here is **pure picture — no voiceover, no dialogue, no on-screen lyrics**. The job is simple: take an existing MP3 and give it visuals. The final video is the storyboard shots played in order over the song. (For voiced/dialogue drama, use `busa-drama` instead — that is its job.)
...
Default to the AirApp for ongoing creative work — give the user the clickable AirApp URL, or run `pnpm --dir content/busa-mv-app dev` for a local preview. Use chat-only mode only when the user explicitly asks for "chat only", "no UI", "纯聊天", or similar.

## Default Flow

1. Open the AirApp (or `pnpm --dir content/busa-mv-app dev` for local preview, which asks you to connect Busabase and select a Space — never an API key).
...
3. Use the app, following the sidebar **下一步 (next step)**:
   ...
   - **角色 (Cast)**: the people on screen. Stable id, role, three-view visual notes, wardrobe, anchors, forbidden drift, and a **character reference card** image. Generate reference cards before storyboard work when consistency matters. No voice profiles (pure-visual MV). Same model as busa-drama.
...
One Folder (`busa-mv`), four Bases, declared in `content/busa-mv-app/app/js/config.js` and
...
```bash
pnpm --dir skills/busa-mv/content/busa-mv-app dev
node skills/busa-mv/scripts/create_sample_project.mjs --apply
node skills/busa-mv/scripts/validate_shot_readiness.mjs
node skills/busa-mv/scripts/export_story_bible.mjs
node skills/busa-mv/scripts/execute_generation_requests.mjs --apply
node skills/busa-mv/scripts/generate_song_draft.mjs --apply
```
