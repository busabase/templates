...
// Reads/writes the operator-provisioned Busa MV Busabase workspace (one
...
function projectFields(row) {
  return {
    project_id: row.project_id || "busa-mv-project",
...
function buildProject({ projectRow, settingsRow, castRows, shotRows, urlOf }) {
  ...
  return {
    project_id: projectRow.project_id || "busa-mv-project",
...
export const busabaseProvider = {
  ...
  async getState() {
    ...
    return {
      ...
      lock: null,
      config_summary: { config_path: "busabase:workspace/busa-mv", is_example: false },
      project,
  ...
  async uploadSong(file, { title = "", artist = "" } = {}) {
    ...
    const [{ assetId }, durationSeconds] = await Promise.all([
      uploadAsset(runtimeClient, file, { context: "busa-mv/song" }),
      durationPromise,
    ]);
  ...
  async uploadShotAsset(shotId, kind, file) {
    await ensureResources();
    const { assetId } = await uploadAsset(runtimeClient, file, { context: `busa-mv/shot-${kind}` });
    const existing = await findRecord("shots", "shot-id", shotId);
    if (!existing) throw new Error(`Unknown shot: ${shotId}`);
    const currentFields = normalizeFields(
      existing.headCommit?.payload || existing.headCommit?.fields || existing.fields,
