...
async function main() {
  ...
  const { assetId } = await uploadAssetFromFile(client, outPath, "audio/wav", "busa-mv/song");
  ...
  await upsert(
    ...
    "project-id",
    current.project_id || "busa-mv-project",
    {
      project_id: current.project_id || "busa-mv-project",
