// Trusted-process Busabase helper shared by every busa-mv skill-root
...
import { appConfig } from "../../content/busa-mv-app/app/js/config.js";
...
export async function connect() {
  ...
  if (!resources.folder || resources.missing.length) {
    throw new Error(
      "Busa MV Busabase resources are not provisioned yet; open the AirApp once (or run its setup) first.",
    );
  }
...
export async function upsert(client, declaredBase, idFieldSlug, idValue, fields, message) {
  ...
  return client.records.changeRequest({
    ...
    fields: normalized,
    message,
    author: "busa-mv-script",
    baseCommitId: existing.headCommitId,
    autoMerge: true,
  });
}

export async function uploadAssetFromBytes(client, bytes, fileName, mimeType, context = "busa-mv/script") {
