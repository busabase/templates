...
Use this schema when reading or writing Busa Listing's Busabase Bases.
Field slugs are kebab-case in Busabase and normalized to snake_case in app
code (`content/busa-listing-app/app/js/providers/busabase-provider.js`,
`content/busa-listing-app/app/js/listing-model.js`). Compliance checks, per-draft compliance
...
## Products (`busa-listing-products`)
...
## Claim Rules (`busa-listing-claim-rules`)
...
## Settings (`busa-listing-settings`)
...
the retired `content/busa-listing-app/server/rules.ts`:
