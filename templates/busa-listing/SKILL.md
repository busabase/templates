...
<table>
  <tr>
    <td width="50%"><img src="assets/screenshots/overview.webp" alt="Busa Listing overview"></td>
    <td width="50%"><img src="assets/screenshots/needs-review.webp" alt="Busa Listing review queue"></td>
  </tr>
  <tr>
  ...
  <tr>
    <td width="50%"><img src="assets/screenshots/checks.webp" alt="Busa Listing compliance checks"></td>
    <td width="50%"><img src="assets/screenshots/drafts.webp" alt="Busa Listing draft workbench"></td>
  </tr>
  <tr>
...
1. Read and follow `$kelly-app-skill-creator` for product behavior, visual quality, responsive layout, and the complete canonical `content/busa-listing-app/` artifact.
...
Resources provision lazily through an idempotent Busabase ChangeRequest the first time the app runs in a Space; see `references/listing-schema.md` for exact field shapes. Compliance scores, the review queue, the recent-activity feed, and metrics are recomputed client-side from the stored rows on every read (`content/busa-listing-app/app/js/listing-model.js`'s `buildSnapshot`/`assembleSnapshot`), so the desk is always fresh regardless of when a browser session loads it relative to the last ingest/checks run.
...
```bash
node skills/busa-listing/scripts/ingest_drafts.mjs payload.json --apply
node skills/busa-listing/scripts/run_checks.mjs --apply
```

## Local App

Default behavior is AirApp-first — give the user the clickable AirApp URL. Start `pnpm --dir content/busa-listing-app dev` only when local preview/debugging is explicitly requested.
...
node skills/busa-listing/scripts/ingest_drafts.mjs payload.json --apply
node skills/busa-listing/scripts/run_checks.mjs --apply
node skills/busa-listing/scripts/execute_decisions.mjs
node skills/busa-listing/scripts/execute_decisions.mjs --apply
node skills/busa-listing/scripts/export_listings.mjs --out exports/
pnpm --dir skills/busa-listing/content/busa-listing-app dev
```

In normal use, invoke `/busa-listing`, let the skill ingest/check what's due, and open the AirApp.
