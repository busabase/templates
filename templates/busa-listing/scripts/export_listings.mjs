#!/usr/bin/env node
// Trusted export step. Busa Listing's AirApp only ever writes a review
...
// changed, from a persisted content/busa-listing-app/.data/listing_snapshot.json + decisions.json
...
import { appConfig } from "../content/busa-listing-app/app/js/config.js";
import { normalizeDraftRow, normalizeProductRow } from "../content/busa-listing-app/app/js/listing-model.js";
...
function listingMarkdown(draft, product, sellerBrand) {
  ...
  lines.push(
    "---",
    "",
    `Exported by busa-listing on ${new Date().toISOString()} (Draft #${draft.ref}, ${draft.draft_id}). Publishing via platform APIs is executed by the agent after approval.`,
    "",
  );
  return lines.join("\n");
}
...
async function main() {
  ...
  if (!resources.folder || resources.missing.length) {
    throw new Error("Busa Listing Busabase resources are not provisioned yet; run the AirApp setup first.");
  }
