#!/usr/bin/env node
// Trusted hand-off step. Busa Family Fund's AirApp is read-only (see
// content/busa-family-fund-app/app/js/config.js — writeProcedures: []); this script is the only
...
async function main() {
  ...
  if (!resources.folder || resources.missing.length) {
    throw new Error("Busa Family Fund Busabase resources are not provisioned yet; run the AirApp setup first.");
  }
  ...
  for (const fields of newBeneficiaries) {
    await client.bases.createChangeRequest({
      ...
      submittedBy: "busa-family-fund-importer",
      autoMerge: true,
    });
  }
