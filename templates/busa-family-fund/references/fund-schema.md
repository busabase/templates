# Busa Family Fund Schema

Use this schema when reading or writing Busa Family Fund's Busabase Bases.
Field slugs are kebab-case in Busabase and normalized to snake_case in app
code (`content/busa-family-fund-app/app/js/providers/busabase-provider.js`,
`content/busa-family-fund-app/app/js/fund-model.js`, `scripts/import_csv.mjs`). The fund snapshot
...
## Beneficiaries (`busa-family-fund-beneficiaries`)
...
## Families (`busa-family-fund-families`)
...
## Income (`busa-family-fund-income`)
...
## Expenses (`busa-family-fund-expenses`)
...
## Settings (`busa-family-fund-settings`)
