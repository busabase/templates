---
name: busa-homework-coach
description: Friendly elementary-school homework tutoring App-in-Skill. Use when the user invokes $busa-homework-coach or /busa-homework-coach, mentions 小学生作业辅导, homework photo tutoring, 拍照讲题, 作业答疑, 错题本, error notebook, mistake-book review, practice paper generation, exam paper analysis, parent/teacher review of AI explanations, or wants a child-friendly UI for step-by-step homework help with parent/teacher review and safety boundaries.
metadata:
  category: education
  tags:
    - risk:local-write
  ...
  busabase:
    template: true
    folderSlug: busa-homework-coach
    resources:
      - questions
      - mistakes
      - papers
...
1. Read and follow `$kelly-app-skill-creator` for product behavior, visual quality, responsive layout, and the complete canonical `content/busa-homework-coach-app/` artifact.
...
Five Bases under one application Folder (`busa-homework-coach`), declared in `content/busa-homework-coach-app/app/js/config.js` and the generated template sidecars under `content/`:
...
There is no upload API and the AirApp's photo box never uploads a file anywhere — it only lets the student pick a local filename and copies a chat prompt asking the agent to analyze it (`content/busa-homework-coach-app/app/app.js`'s `renderPhotoBox()`/`data-copy-prompt="photo"`). The agent does the actual work in the same chat session (OCR/vision reasoning, drafting the child-facing explanation, identifying the mistake) and then calls the trusted script below with its own Busabase credentials to record the result:

```bash
node skills/busa-homework-coach/scripts/record_homework.mjs --file payload.json --apply
```
...
Default behavior is AirApp-first — give the user the clickable AirApp URL. Start `pnpm --dir content/busa-homework-coach-app dev` only when local preview/debugging is explicitly requested.
...
4. When a review's decision is `request_changes`, the linked review row's status stays `changes_requested`, which is exactly the retired app's "queued agent task" (see `pendingAgentTasks()` in `content/busa-homework-coach-app/app/js/homework-model.js`). Redraft the explanation, mistake card, or paper plan, then re-run `scripts/record_homework.mjs` with the updated content and the same review id.
...
```bash
node skills/busa-homework-coach/scripts/record_homework.mjs --file payload.json --apply
node skills/busa-homework-coach/scripts/execute_decisions.mjs --apply
pnpm --dir skills/busa-homework-coach/content/busa-homework-coach-app dev
```
