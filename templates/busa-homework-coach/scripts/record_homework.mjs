...
// actually enters this skill. Busa Homework Coach has no upload API route
// (grep of the retired content/busa-homework-coach-app/server/hono.ts confirms it: only /api/state,
// /api/provider, /api/onboarding/complete, /api/decision) and the retired
// browser's "photo box" (content/busa-homework-coach-app/app.js renderPhotoBox()) never uploaded a file
...
// into the local snapshot file (with a temporary content/busa-homework-coach-app/.data/agent.lock).
...
import { appConfig } from "../content/busa-homework-coach-app/app/js/config.js";
import {
  baseMistakeFields,
  basePaperFields,
  baseQuestionFields,
...
} from "../content/busa-homework-coach-app/app/js/homework-model.js";
...
async function main() {
  ...
  if (!resources.folder || resources.missing.length) {
    throw new Error("Busa Homework Coach Busabase resources are not provisioned yet; run the AirApp setup first.");
  }
    ...
    for (const item of items) {
      ...
      if (existing) {
        await client.records.changeRequest({
          ...
          author: "busa-homework-coach-recorder",
        ...
        await client.bases.createChangeRequest({
          ...
          submittedBy: "busa-homework-coach-recorder",
          autoMerge: true,
        });
      }
    }
