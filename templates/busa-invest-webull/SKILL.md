...
Busa Invest (Webull) is a Busabase Cloud App-in-Skill. Its canonical product
...
connection bootstrap. Use this skill as the operator's read-only Webull portfolio
...
1. Read and follow `$kelly-app-skill-creator` for product behavior, visual
   quality, responsive layout, and the complete canonical `content/busa-invest-webull-app/` artifact.
...
Three Bases under one application Folder (`busa-invest-webull`), declared in
`content/busa-invest-webull-app/app/js/config.js` and the generated template sidecars under `content/`:
...
- `positions`: holdings per account (`position_id` = `account_id:symbol`,
  ...
  `content/busa-invest-webull-app/app/js/webull-model.js` (`assembleSnapshot`), never stored.
...
- `lib/data-provider/webull.ts`'s field-mapping and credential-resolution
  ...
  `content/busa-invest-webull-app/app/js/webull-model.js`, shared by both the browser (for
...
Start `pnpm --dir content/busa-invest-webull-app dev` only when local preview/debugging is explicitly
...
`content/busa-invest-webull-app/app/js/webull-model.js` (ported verbatim from the retired
`content/busa-invest-webull-app/server/insights.ts`): `single_position_concentration`,
...
`content/busa-invest-webull-app/app/js/config.js`, `content/busa-invest-webull-app/app/js/webull-model.js`, or
