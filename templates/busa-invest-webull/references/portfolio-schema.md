# Busa Invest (Webull) Portfolio Schema

This schema describes both the Busabase `accounts`/`positions`/`settings`
Bases (declared in `content/busa-invest-webull-app/app/js/config.js`) and the in-memory snapshot that
`assembleSnapshot()` in `content/busa-invest-webull-app/app/js/webull-model.js` computes from them at
read time. Keep the shape stable so the app, `scripts/sync_webull.mjs`, and
`content/busa-invest-webull-app/app/js/webull-model.js` (the ported Webull field-mapping adapter) can
...
- `accounts` Base (slug `busa-invest-webull-accounts`): one row per
  `mapAccount()` output — `account-id`, `account-type`, `display-name`,
  `currency`, `net-liquidation`, `total-cash`, `buying-power`.
- `positions` Base (slug `busa-invest-webull-positions`): one row per
...
- `settings` Base (slug `busa-invest-webull-settings`): two rows keyed by
...
`source` (e.g. `"busa-invest-webull"`) is optional metadata and may be included.
