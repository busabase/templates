#!/usr/bin/env node
// Trusted hand-off step. Busa Invest (Webull)'s AirApp is read-only (see
// content/busa-invest-webull-app/app/js/config.js — readOnly: true, writeProcedures: []); this script is
...
// content/busa-invest-webull-app/app/js/webull-model.js (mapAccount/mapPosition/resolveWebullCredentials),
...
import { appConfig } from "../content/busa-invest-webull-app/app/js/config.js";
import {
  mapAccount,
  mapPosition,
  resolveWebullCredentials,
} from "../content/busa-invest-webull-app/app/js/webull-model.js";

const SKILL_DIR = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "..");

function fail(message) {
  console.error(`busa-invest-webull sync: ${message}`);
  process.exit(1);
}

// ── Local config/env discovery, ported from the retired content/busa-invest-webull-app/server/store.ts ──
...
function configSearchPaths() {
  ...
  paths.push(path.join(process.env.HOME || "", ".config", "busa-invest-webull", "config.json"));
  return paths;
}
...
function envSearchPaths() {
  ...
  paths.push(path.join(process.env.HOME || "", ".config", "busa-invest-webull", ".env"));
  return paths;
}
  ...
  if (!credentials.appKey || !credentials.appSecret) {
    fail(
      [
        ...
        "local env file (e.g. skills/busa-invest-webull/.env.local), then re-run.",
        "",
...
async function upsert(client, declared, idValue, existingByKey, fields, message) {
  ...
  if (existing?.__recordId) {
    return client.records.changeRequest({
      ...
      message,
      author: "busa-invest-webull-sync",
  ...
  return client.bases.createChangeRequest({
    ...
    fields: toBusabaseFields(fields),
    message,
    submittedBy: "busa-invest-webull-sync",
    autoMerge: true,
  });
}
...
async function main() {
  ...
  if (!resources.folder || resources.missing.length) {
    throw new Error("Busa Invest (Webull) Busabase resources are not provisioned yet; run the AirApp setup first.");
  }
  ...
  await upsert(
    ...
    configByKey,
    {
      record_id: "config",
      kind: "config",
      name: "Busa Invest (Webull) config",
  ...
  await upsert(
    ...
    "Mark Busa Invest (Webull) onboarding complete",
  );
