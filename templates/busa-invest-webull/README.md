...
Busa Invest (Webull) is a Busabase Cloud App-in-Skill dashboard that
...
```bash
pnpm --dir skills/busa-invest-webull/content/busa-invest-webull-app dev
```
...
Three Bases under one application Folder (`busa-invest-webull`): `accounts`,
...
`lib/data-provider/webull.ts` into `content/busa-invest-webull-app/app/js/webull-model.js`, and upserts
