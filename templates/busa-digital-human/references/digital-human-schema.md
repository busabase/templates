# Busa Digital Human Schema

Use this reference when changing `busa-digital-human`.

## Scope

`busa-digital-human` is mostly a curated reference/comparison desk, not a
...
`content/busa-digital-human-app/app/js/digital-human-model.js`. The retired `content/busa-digital-human-app/server/index.ts`'s
...
code (`content/busa-digital-human-app/app/js/providers/busabase-provider.js`,
`content/busa-digital-human-app/app/js/digital-human-model.js`). There is no delete operation anywhere
in this skill.

### QA Decisions (`busa-digital-human-qa-decisions`)
...
`content/busa-digital-human-app/app/js/digital-human-model.js` exports plain constants, shared verbatim
...
- `METRICS_STATIC`: `target_latency_ms`, `current_latency_ms`,
  ...
  `status` field (replaces the retired `content/busa-digital-human-app/server/demo.ts`'s hardcoded `6`/
...
`content/busa-digital-human-app/app/js/digital-human-model.js`:
...
`content/busa-digital-human-app/app/js/providers/demo-provider.js` returns `buildSnapshot(QA_CHECKS,
...)` with an empty decisions map and a fixed `generated_at` of
`2026-07-07T09:30:00.000Z`. Matching the retired `content/busa-digital-human-app/app.js`'s behavior, a
