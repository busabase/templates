...
Busa Digital Human is a Busabase Cloud App-in-Skill. Its canonical product
...
connection bootstrap. It is the operator's digital-human solution desk: pick the
...
`busa-clm`'s approval queue uses. There is no separate decisions-log bucket.
...
- Keep all business logic, script review, safety wording, telemetry, and QA
  gates in the operator's local workflow.
...
1. Read and follow `$kelly-app-skill-creator` for product behavior, visual
   quality, responsive layout, and the complete canonical `content/busa-digital-human-app/` artifact.
...
3. Open the AirApp (or `pnpm --dir content/busa-digital-human-app dev` for local preview) and use demo
...
One Base under one application Folder (`busa-digital-human`), declared in
`content/busa-digital-human-app/app/js/config.js` and the generated template sidecars under `content/`:
...
- `qa-decisions`: one row per launch-QA-check decision, keyed by the curated
  ...
  `content/busa-digital-human-app/app/js/digital-human-model.js` -- they are not stored in Busabase
...
Start `pnpm --dir content/busa-digital-human-app dev` only when local preview/debugging is explicitly
requested.
...
- the skill contains the complete canonical `content/busa-digital-human-app/` project and
  `pnpm --dir content/busa-digital-human-app dev` remains supported;
...
- Overview, QA review, Studio, Vendors, and Help & Settings render on desktop
  and phone widths;
- `pnpm --dir content/busa-digital-human-app run check` and `node --test` pass.
