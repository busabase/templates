# Busa Messenger Schema

Use this schema when reading or writing Busa Messenger's Busabase Bases.
Field slugs are kebab-case in Busabase and normalized to snake_case in app
code (`content/busa-messenger-app/app/js/providers/busabase-provider.js`,
`content/busa-messenger-app/app/js/messenger-model.js`). Per-conversation/per-account rollups
...
## Accounts (`busa-messenger-accounts`)
...
## Conversations (`busa-messenger-conversations`)
...
## Messages (`busa-messenger-messages`)
...
## Sync Log (`busa-messenger-sync-log`)
...
## Replies (`busa-messenger-replies`)
...
## Settings (`busa-messenger-settings`)
