...
// write target changed, from content/busa-messenger-app/.data/messages_snapshot.json to Busabase's
...
import { appConfig } from "../content/busa-messenger-app/app/js/config.js";
import { API_CONNECTORS, SECRET_ENV_KEYS } from "../content/busa-messenger-app/app/js/messenger-model.js";
...
async function upsertRow(client, declared, existing, idField, idValue, fields, message, apply) {
  ...
  await client.bases.createChangeRequest({
    ...
    fields: normalized,
    message,
    submittedBy: "busa-messenger-sync",
    autoMerge: true,
  });
  return "created";
}
...
async function main() {
  ...
  if (!resources.folder || resources.missing.length) {
    throw new Error("Busa Messenger Busabase resources are not provisioned yet; run the AirApp setup first.");
  }
  ...
  if (!accounts.length) {
    console.log("Busa Messenger sync: no API-connector accounts configured yet.");
  ...
  if (!ready.length) {
    console.log("Busa Messenger sync: no connector tokens found, nothing was synced.");
