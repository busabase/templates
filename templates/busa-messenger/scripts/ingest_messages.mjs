...
// content/busa-messenger-app/.data/messages_snapshot.json to Busabase records. The retired script
...
function fail(message) {
  console.error(`busa-messenger ingest: ${message}`);
  process.exit(1);
}
...
async function upsertRow(client, declared, existing, idField, idValue, fields, message, apply) {
  ...
  await client.bases.createChangeRequest({
    ...
    fields: normalized,
    message,
    submittedBy: "busa-messenger-ingest",
    autoMerge: true,
  });
  return "created";
}
  ...
  for (const file of payloadFiles) {
    ...
    if (payload.account) {
      ...
      await upsertRow(
        ...
        input.account_id,
        fields,
        `Upsert busa-messenger account ${input.account_id}`,
        apply,
      );
