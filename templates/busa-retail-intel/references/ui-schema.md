# Busa Retail Intel Schema

Use this schema when reading or writing Busa Retail Intel's
Busabase Bases. Field slugs are kebab-case in Busabase and normalized to
snake_case in app code (`content/busa-retail-intel-app/app/js/providers/busabase-provider.js`,
`content/busa-retail-intel-app/app/js/retail-model.js`). The batch rollup metrics
...
## Signals (`busa-retail-intel-signals`)
...
## Actions (`busa-retail-intel-actions`)
...
## Drafts (`busa-retail-intel-drafts`)
...
## Sources (`busa-retail-intel-sources`)
...
## Settings (`busa-retail-intel-settings`)
...
(`operationForDecision()` in `content/busa-retail-intel-app/app/js/retail-model.js`) for every
