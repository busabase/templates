...
(`content/busa-money-app/app/js/providers/busabase-provider.js`, `content/busa-money-app/app/js/money-model.js`).

## Accounts (`busa-money-accounts`)
...
## Transactions (`busa-money-transactions`)
...
## Invoice Matches (`busa-money-invoice-matches`)
...
## Settings (`busa-money-settings`)

One row per `kind`, looked up by `record-id`:

| `record-id` | `kind` | `payload` (JSON) |
| --- | --- | --- |
| `busa-money-accounts` | `accounts` | `{accounts: [{account_id, provider, display_name, entity, currency, secret_envs, secrets_ready}]}` — non-secret account inventory only |
| `busa-money-lock` | `lock` | not JSON-wrapped: fields `locked` (bool), `owner`, `message` live directly on the row |
