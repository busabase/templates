...
Busa Money is a Busabase Cloud App-in-Skill. Its canonical product surface
...
bootstrap. Use this skill as the operator's money ledger operator: it aggregates
...
1. Read and follow `$kelly-app-skill-creator` for product behavior, visual
   quality, responsive layout, and the complete canonical `content/busa-money-app/` artifact.
...
<table>
  <tr>
    <td width="50%"><img src="assets/screenshots/overview.webp" alt="Busa Money overview"></td>
    <td width="50%"><img src="assets/screenshots/ledger.webp" alt="Busa Money total ledger"></td>
  </tr>
  <tr>
  ...
  <tr>
    <td width="50%"><img src="assets/screenshots/accounts.webp" alt="Busa Money accounts"></td>
    <td width="50%"><img src="assets/screenshots/invoices.webp" alt="Busa Money invoice matching"></td>
  </tr>
  <tr>
  ...
  <tr>
    <td width="50%"><img src="assets/screenshots/detail.webp" alt="Busa Money invoice exception detail"></td>
  </tr>
  <tr>
...
- Require explicit user approval before any remote mutation. Normal Busa Money operation is read-only aggregation unless the user asks for a specific approved action, and that action happens outside the AirApp.

## Busabase Resources

Five Bases under one application Folder (`busa-money`), declared in
`content/busa-money-app/app/js/config.js` and the generated template sidecars under `content/`:
...
- `settings`: one row per `kind` — `busa-money-accounts` (configured account list, non-secret) and `busa-money-lock`.
...
On invocation, check the `busa-money-accounts` settings row for readiness.
...
Start `pnpm --dir content/busa-money-app dev` only when local preview/debugging is explicitly
requested.
...
Invoice matching lives inside Busa Money rather than a separate skill until it becomes a full invoice-generation or tax-export workflow. Write imported invoice metadata into the `invoices` Base and matching decisions into `invoice-matches`; do not store private invoice PDFs in git.
