#!/usr/bin/env node
// Trusted hand-off step. Busa Product Hub's AirApp never ingests a product
// itself -- the browser cannot read an arbitrary local file path, a
// marketplace export, an inventory CSV, or a certificate PDF. This script
// reads a JSON payload file shaped like the retired
// content/busa-product-hub-app/.data/products_snapshot.json contract
// ({ seller, products: [...], channel_matrix: [...], inventory: [...],
// review_items: [...], certificates: [...] }) and upserts every row into
// Busabase by natural key (product_id, channel_id, inventory_id, item_id,
// cert_id) so re-ingests are idempotent. The seller/platforms/warehouses/
// review_policy/sync summary is upserted into the single Settings row when
// `seller`, `platforms`, `warehouses`, `review_policy`, or `sync` are
...
import { appConfig } from "../content/busa-product-hub-app/app/js/config.js";
import {
  certificateToFields,
  channelToFields,
  inventoryToFields,
...
({ seller, products: [...], channel_matrix: [...], inventory: [...],
review_items: [...], certificates: [...] }) and upserts every row into
Busabase by natural key. Without --apply this is a dry run that only prints
...
const toBusabaseFields = (fields) =>
  Object.fromEntries(
    Object.entries(fields).map(([key, value]) => [
      key.replaceAll("_", "-"),
      // A relation field's value is an array of target record ids -- coercing
      // it to a string would collapse a single-element array to a bare id
      // string (JS's default Array#toString), the wrong wire shape.
...
async function upsert(client, declared, existing, fields, message, apply) {
  ...
  if (existing) {
    await client.records.changeRequest({
      ...
      fields: normalized,
      message,
      author: "busa-product-hub-ingest",
  ...
  await client.bases.createChangeRequest({
    ...
    fields: normalized,
    message,
    submittedBy: "busa-product-hub-ingest",
    autoMerge: true,
  });
  return "created";
}
...
async function main() {
  ...
  const incomingCertificates = payload.certificates || [];
  ...
  if (!resources.folder || resources.missing.length) {
    throw new Error("Busa Product Hub Busabase resources are not provisioned yet; run the AirApp setup first.");
  }
  const declared = (key) => resources.bases.find((base) => base.key === key);

  const [existingProducts, existingChannels, existingInventory, existingReviewItems, existingCertificates, existingSettings] =
    await Promise.all([
      ...
      readAll(client, declared("certificates")),
  ...
  // Certificates have no declared natural-key field (see product-hub-model.js
  // -- identity is the Busabase record id itself). A real certificate number
  // is the closest thing to a business key, so re-ingests key on
  // product_id + cert_type + cert_number to stay idempotent the same way
  // channels/inventory key on a synthesized id when the payload has none.
  //
  // `product` is a real relation field (unlike channels/inventory's plain-text
  // product-id), so it needs the target's actual Busabase record id, not the
  ...
  let certsUpdated = 0;
  let certsSkipped = 0;
  for (const input of incomingCertificates) {
    if (!input.product_id) throw new Error("Every certificate needs a product_id");
    if (!input.cert_number) throw new Error("Every certificate needs a cert_number");
    const targetProduct = productsById.get(input.product_id);
    if (!targetProduct) {
      console.log(`skip certificate ${input.cert_number} — no product ${input.product_id} found; ingest it first`);
      certsSkipped += 1;
      continue;
    }
    const key = certKey(input);
    const existing = certsByKey.get(key);
    const fields = certificateToFields({ ...existing, ...input, product_id: [targetProduct.__recordId] });
    const result = await upsert(
      client,
      declared("certificates"),
      existing,
      fields,
      `${existing ? "Update" : "Ingest"} certificate ${input.cert_number} (${input.product_id})`,
      apply,
    );
    if (result === "created" || result === "would_create") certsCreated += 1;
    else certsUpdated += 1;
    console.log(
      `${apply ? result : `would ${result.replace("would_", "")}`} certificate ${input.cert_number} — ${input.cert_type || ""}`,
    );
  }
  summarize(certsCreated, certsUpdated, "certificates");
  if (certsSkipped) console.log(`certificates: ${certsSkipped} skipped (unresolved product)`);
  ...
  if (!apply) {
    ...
    console.log("Wrote products/channels/inventory/review_items/certificates to Busabase.");
  }
}
