#!/usr/bin/env node
// Trusted hand-off step. Busa Product Hub's AirApp only ever records a review
...
// the approved operation outside the app and records the outcome. The same
// review queue also carries `spec_claim` items (a specification the agent
// extracted from a document without a clear page/section citation); those
// resolve the same way -- a human decision, then this script records the
// execution marker, never the value itself.
//
...
verify_spec_claim / maintain_block / archive_product / request_revision) for
each. With --apply
  ...
  for (const row of decided) {
    ...
    if (apply) {
      await client.records.changeRequest({
        ...
        author: "busa-product-hub-execute-decisions",
