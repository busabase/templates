...
if (!configText.includes('deployment: "cloud"')) throw new Error("Busa Product Hub must be Cloud-only");
if (/BUSA_PRODUCT_HUB_CONFIG|local-file-provider|config\.local\.json|config\.example\.json/.test(configText)) {
...
console.log(`Busa Product Hub AirApp checks OK (${files.length} files)`);
