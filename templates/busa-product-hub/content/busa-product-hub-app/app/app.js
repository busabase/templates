...
import {
  ...
  statusForVerdict,
} from "./js/product-hub-model.js?v=0.1.0";
...
const state = {
  ...
  notice: "",
  lang: normalizeLang(
    new URLSearchParams(location.search).get("lang") || localStorage.getItem("busa-product-hub-language") || "auto",
  ),
...
const SIDEBAR_COLLAPSED_STORAGE_KEY = "busa-product-hub.sidebarCollapsed";
...
const els = {
  ...
  certCount: document.querySelector("#count-cert"),
  language: document.querySelector("#language"),
};

const PAGINATED_KEYS = ["products", "channels", "inventory", "review", "certificates"];
...
async function loadState() {
  ...
  state.settings = data;
  applyDemoRoute();
  window.dispatchEvent(new CustomEvent("busa-product-hub:state", { detail: data }));
  render();
}
...
function certificates() {
  return state.snapshot?.certificates || [];
}
...
function renderShell() {
  ...
  if (els.certCount) {
    els.certCount.textContent = String((metrics.expiring_cert_count || 0) + (metrics.expired_cert_count || 0));
  }
  ...
  els.content.innerHTML = `
    ...
    <section class="panel">
      <div class="panel-head">
        <h2>${t("certsExpiringSoon")}</h2>
        <a href="#/certificates">${t("certificates")}</a>
      </div>
      ${
        urgentCerts.length
          ? `<div class="table-wrap"><table>
              <thead><tr><th>${t("product")}</th><th>${t("certType")}</th><th>${t("expiryDate")}</th><th>${t("status")}</th></tr></thead>
              <tbody>
                ${urgentCerts.map((item) => certRow(item)).join("")}
              </tbody>
            </table></div>`
          : `<p class="muted">${t("noCertsUrgent")}</p>`
      }
    </section>
...
function renderProductDetail(productId) {
  ...
  els.content.innerHTML = `
    ...
    <section class="panel">
      <div class="panel-head"><h2>${t("certificates")}</h2><a href="#/certificates">${t("certificates")}</a></div>
      ${
        certRows.length
          ? `<div class="table-wrap"><table>
              <thead><tr><th>${t("certType")}</th><th>${t("certNumber")}</th><th>${t("issuedDate")}</th><th>${t("expiryDate")}</th><th>${t("status")}</th></tr></thead>
              <tbody>
                ${certRows
                  .map(
                    (item) => `<tr>
...
// A certificate's `cert_status` (valid/expiring_soon/expired) is annotated by
...
function renderCertificates() {
  ...
  setPage(
    t("certificates"),
    `${(metrics.expiring_cert_count || 0) + (metrics.expired_cert_count || 0)} ${t("certsExpiringSoon").toLowerCase()}`,
  );
  els.content.innerHTML = `<section class="panel">
    <div class="panel-head"><h2>${t("certificates")}</h2><span class="muted">${t("sortedSoonestFirst")}</span></div>
    <div class="table-wrap"><table>
      <thead><tr><th>${t("product")}</th><th>${t("certType")}</th><th>${t("certNumber")}</th><th>${t("issuedDate")}</th><th>${t("expiryDate")}</th><th>${t("status")}</th><th>${t("sourceNote")}</th></tr></thead>
      <tbody>
        ${certificates()
          .map(
            (item) => `<tr>
...
function render() {
  ...
  else if (state.route.view === "certificates") renderPagedList(renderCertificates, "certificates");
...
function bindEvents() {
  ...
  els.language.addEventListener("change", () => {
    ...
    localStorage.setItem("busa-product-hub-language", state.lang);
