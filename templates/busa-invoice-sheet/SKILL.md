---
name: busa-invoice-sheet
description: Extract invoices, receipts, credit notes, statements, PDFs, images, docs, and spreadsheet-like invoice exports into a reviewable table with field confidence, line items, approval decisions, and CSV/JSON export. Use when the user invokes /busa-invoice-sheet or $busa-invoice-sheet, asks for "Invoice转表格", invoice OCR, receipt-to-spreadsheet, invoice data extraction, bookkeeping import prep, or a Lido-style Extract Data workflow with a Busabase-backed App-in-Skill UI.
metadata:
  category: finance
  tags:
    - risk:local-write
...
<table>
  <tr>
    <td width="33%"><img src="assets/screenshots/overview.webp" alt="Busa Invoice Sheet spreadsheet extraction desk"></td>
    <td width="33%"><img src="assets/screenshots/detail.webp" alt="Busa Invoice Sheet invoice detail review"></td>
    <td width="33%"><img src="assets/screenshots/extract-data.webp" alt="Busa Invoice Sheet Extract Data upload modal"></td>
  </tr>
  <tr>
...
1. Read and follow `$kelly-app-skill-creator` for product behavior, visual quality, responsive layout, and the complete canonical `content/busa-invoice-sheet-app/` artifact.
...
6. Give the user the AirApp URL (or start local preview with `pnpm --dir content/busa-invoice-sheet-app dev` if explicitly requested).
...
Two Bases under one application Folder (`busa-invoice-sheet`), declared in `content/busa-invoice-sheet-app/app/js/config.js` and the generated template sidecars under `content/`:
...
Default behavior is AirApp-first — give the user the clickable AirApp URL. Start `pnpm --dir content/busa-invoice-sheet-app dev` only when local preview/debugging is explicitly requested.
...
```bash
node skills/busa-invoice-sheet/scripts/import_batch.mjs --file batch.json --apply
node skills/busa-invoice-sheet/scripts/export_decisions.mjs --apply
pnpm --dir skills/busa-invoice-sheet/content/busa-invoice-sheet-app dev
```
