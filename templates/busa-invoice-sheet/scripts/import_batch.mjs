#!/usr/bin/env node
// Trusted ingestion step. Busa Invoice Sheet's AirApp is read/decision-only
...
// content/busa-invoice-sheet-app/app/js/invoice-model.js), and upserts each invoice as a Busabase
...
import { appConfig } from "../content/busa-invoice-sheet-app/app/js/config.js";
import { baseInvoiceFields, validateInvoicesShape } from "../content/busa-invoice-sheet-app/app/js/invoice-model.js";
...
async function main() {
  ...
  if (!resources.folder || resources.missing.length) {
    throw new Error("Busa Invoice Sheet Busabase resources are not provisioned yet; run the AirApp setup first.");
  }
  ...
  for (const invoice of invoices) {
    ...
    if (existing) {
      await client.records.changeRequest({
        ...
        author: "busa-invoice-sheet-importer",
      ...
      await client.bases.createChangeRequest({
        ...
        submittedBy: "busa-invoice-sheet-importer",
        autoMerge: true,
      });
      created += 1;
    }
