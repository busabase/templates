# Busa Portfolio Health Schema

Use this schema when reading or writing Busa Portfolio Health's Busabase
Bases. Field slugs are kebab-case in Busabase and normalized to snake_case
in app code (`content/busa-portfolio-health-app/app/js/providers/busabase-provider.js`,
`content/busa-portfolio-health-app/app/js/portfolio-model.js`). `insights` (totals, per-contract repayment
...
## Contracts (`busa-portfolio-health-contracts`)
...
| --- | --- | --- |
| `busa-portfolio-health-config` | `config` | `{base_currency, fund_name, risk_policy: {lag_watch_pp, lag_high_pp, revenue_decline_pct}}` |

If no `config` row exists, the app falls back to defaults
(`content/busa-portfolio-health-app/app/js/portfolio-model.js`'s `DEFAULT_RISK_POLICY`
...
`computeInsights(contracts, riskPolicy)` in `content/busa-portfolio-health-app/app/js/portfolio-model.js`:
...
`content/busa-portfolio-health-app/app/js/providers/busabase-provider.js`, using
