...
metadata:
  ...
  busabase:
    template: true
    folderSlug: busa-portfolio-health
    resources:
      - contracts
      - settings
    risk: local-write
...
Busa Portfolio Health is a Busabase Cloud App-in-Skill. Its canonical
...
(`content/busa-portfolio-health-app/app/js/portfolio-model.js`, ported from the retired
`content/busa-portfolio-health-app/server/insights.ts`); the human flags/clears/annotates a contract
...
`busa-lead-funnel`'s kanban stage moves work.
...
1. Read and follow `$kelly-app-skill-creator` for product behavior, visual
   quality, responsive layout, and the complete canonical `content/busa-portfolio-health-app/` artifact.
...
- Contracts are never created by the AirApp; they enter Busabase through an
  ...
  and `busa-lead-funnel`'s leads enter through an upstream process the app
...
Two Bases under one application Folder (`busa-portfolio-health`), declared
in `content/busa-portfolio-health-app/app/js/config.js` and the generated template sidecars under `content/`:
...
`content/busa-portfolio-health-app/app/js/portfolio-model.js` (`computeInsights`) is ported verbatim from
the retired `content/busa-portfolio-health-app/server/insights.ts`:
...
Start `pnpm --dir content/busa-portfolio-health-app dev` only when local preview/debugging is explicitly
requested.
...
- the skill contains the complete canonical `content/busa-portfolio-health-app/` project and
  `pnpm --dir content/busa-portfolio-health-app dev` remains supported;
...
- `pnpm --dir content/busa-portfolio-health-app run check` and `node --test` pass.
