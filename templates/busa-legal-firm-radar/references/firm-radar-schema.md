...
code (`content/busa-legal-firm-radar-app/app/js/providers/busabase-provider.js`,
`content/busa-legal-firm-radar-app/app/js/firm-radar-model.js`). Metrics and the recent-activity feed are
...
`busa-legal-casebase-ingest`, `revise` maps status back to `needs_review`
...
## Items (`busa-legal-firm-radar-items`)
...
## Entities (`busa-legal-firm-radar-entities`)
...
## Checks (`busa-legal-firm-radar-checks`)
...
## Settings (`busa-legal-firm-radar-settings`)
