...
metadata:
  ...
  busabase:
    template: true
    folderSlug: busa-legal-firm-radar
    resources:
      - items
      - entities
      - checks
...
1. Read and follow `$kelly-app-skill-creator` for product behavior, visual quality, responsive layout, and the complete canonical `content/busa-legal-firm-radar-app/` artifact.
...
Four Bases under one application Folder (`busa-legal-firm-radar`), declared in `content/busa-legal-firm-radar-app/app/js/config.js` and the generated template sidecars under `content/`:
...
Resources provision lazily through an idempotent Busabase ChangeRequest the first time the app runs in a Space; see `references/firm-radar-schema.md` for exact field shapes. Metrics and the recent-activity feed are recomputed client-side from the stored rows on every read (`content/busa-legal-firm-radar-app/app/js/firm-radar-model.js`'s `buildSnapshot`/`assembleSnapshot`), so the desk is always fresh regardless of when a browser session loads it relative to the last import/decision.
...
```bash
node skills/busa-legal-firm-radar/scripts/import_metrics.mjs payload.json --apply
```

## Local App

Default behavior is AirApp-first — give the user the clickable AirApp URL. Start `pnpm --dir content/busa-legal-firm-radar-app dev` only when local preview/debugging is explicitly requested.
...
```bash
node skills/busa-legal-firm-radar/scripts/import_metrics.mjs payload.json --apply
node skills/busa-legal-firm-radar/scripts/execute_decisions.mjs
node skills/busa-legal-firm-radar/scripts/execute_decisions.mjs --apply
node skills/busa-legal-firm-radar/scripts/export_management_report.mjs --out exports/
pnpm --dir skills/busa-legal-firm-radar/content/busa-legal-firm-radar-app dev
```

In normal use, invoke `/busa-legal-firm-radar`, let the skill import what's due, and open the AirApp.
