...
Busa Launch is a Busabase Cloud App-in-Skill. Its canonical product surface
...
bootstrap. Use this skill as the operator's product-launch command desk: it keeps a
...
whatever the operator feeds it — product docs, positioning notes, a target date —
...
channels (for example `busa-email` for press/launch email, or a channel
...
1. Read and follow `$kelly-app-skill-creator` for product behavior, visual
   quality, responsive layout, and the complete canonical `content/busa-launch-app/` artifact.
...
busa-launch implements the **RAMP** launch discipline — **Research / Assemble / Mobilize / Prove** — as its four item `phase`s and as the structure of the checklist. Every launch task or asset belongs to exactly one phase. The capability taxonomy the skill draws on within each phase:
...
<table>
  <tr>
    <td width="50%"><img src="assets/screenshots/overview.webp" alt="Busa Launch overview"></td>
    <td width="50%"><img src="assets/screenshots/assets.webp" alt="Busa Launch assets queue"></td>
  </tr>
  <tr>
  ...
  <tr>
    <td width="50%"><img src="assets/screenshots/checklist.webp" alt="Busa Launch checklist"></td>
    <td width="50%"><img src="assets/screenshots/launchday.webp" alt="Busa Launch launch-day runbook"></td>
  </tr>
  <tr>
...
- The skill may read sources the operator provides, assemble the checklist, draft assets/submissions/pitches, score readiness, and write it all to Busabase.
...
Four Bases under one application Folder (`busa-launch`), declared in
`content/busa-launch-app/app/js/config.js` and the generated template sidecars under `content/`:
...
- `settings`: one row per `kind` — `busa-launch-profile` (product/launch profile, style tone, press lists, readiness policy, configured channels) and `busa-launch-lock`.
...
On invocation, check the `busa-launch-profile` settings row for readiness.
...
Start `pnpm --dir content/busa-launch-app dev` only when local preview/debugging is explicitly
requested.
...
2. When the operator feeds new material (product docs, positioning, a date): write RAMP items to Busabase with `phase`, per-item `readiness`, `proposed_action`, and an editable `draft` for assets/submissions. The readiness gate, phase progress, and channel status recompute automatically on every read.
3. Give the operator the AirApp URL (or local preview URL) to review the checklist, the asset queue, and the launch-day runbook.
4. For an item moved to `changes_requested`, re-draft it per the review comment and write it back to `needs_review`.
5. On "execute" / "submit approved": run `node scripts/execute_decisions.mjs --apply` to re-read approved items from Busabase and mark them `done`, then perform the actual submissions/sends only through the corresponding skill (for example `$busa-email` for a press pitch or launch email), one item at a time.
