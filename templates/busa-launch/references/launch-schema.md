...
Use this schema when reading or writing Busa Launch's Busabase Bases. Field
slugs are kebab-case in Busabase and normalized to snake_case in app code
(`content/busa-launch-app/app/js/providers/busabase-provider.js`, `content/busa-launch-app/app/js/launch-model.js`).
...
## Settings (`busa-launch-settings`)

One row per `kind`, looked up by `record-id`:

| `record-id` | `kind` | `payload` (JSON) |
| --- | --- | --- |
| `busa-launch-profile` | `profile` | `{product: {name, tagline, homepage, category}, launch: {target_date, timezone}, style_tone, press_lists, readiness_policy: {block_on, min_ship_ratio}, channels: [{channel_id, type, display_name, handoff_skill, secret_envs, secrets_ready}]}` |
| `busa-launch-lock` | `lock` | not JSON-wrapped: fields `locked` (bool), `owner`, `message` live directly on the row |
