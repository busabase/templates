...
function demoTickets() {
  return [
    ...
    ticket(
      ...
      "needs_review",
      "refund",
      "Refund request within the 30-day window (charged yesterday). KB confirms eligibility, but refunds are approval-required — do not send until the operator approves.",
...
function localizeSnapshotZh(snapshot) {
  ...
  const reasons = {
    "tk-ochoa-refund":
      "退款请求在 30 天窗口内（昨天扣款）。知识库确认符合条件，但退款需人工审批 —— 未经你批准不得发送。",
...
export const demoProvider = {
  ...
  async getState() {
    ...
    return {
      ...
      lock: null,
      config_summary: {
        config_path: "demo://busa-support/config.json",
        is_example: false,
        sla_policy: {
