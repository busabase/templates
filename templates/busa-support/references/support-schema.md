...
Use this schema when reading or writing Busa Support's Busabase Bases. Field
slugs are kebab-case in Busabase and normalized to snake_case in app code
(`content/busa-support-app/app/js/providers/busabase-provider.js`, `content/busa-support-app/app/js/support-model.js`).
...
## Accounts (`busa-support-accounts`)
...
## Messages (`busa-support-messages`)
...
## Knowledge Base (`busa-support-knowledge-base`)
...
## Sync Log (`busa-support-sync-log`)
...
## Settings (`busa-support-settings`)
...
`content/busa-support-app/app/js/support-model.js` returns `{verdict, score, summary, checks}`:
