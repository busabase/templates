...
Use this reference when changing `busa-clm`.

## Scope

`busa-clm` is intentionally light. It is a contract operations tracker:
...
Use this schema when reading or writing Busa CLM's Busabase Bases. Field
slugs are kebab-case in Busabase and normalized to snake_case in app code
(`content/busa-clm-app/app/js/providers/busabase-provider.js`, `content/busa-clm-app/app/js/clm-model.js`).
There is no delete operation anywhere in this skill.

### Contracts (`busa-clm-contracts`)
...
### Obligations (`busa-clm-obligations`)
...
`content/busa-clm-app/app/js/clm-model.js`'s `computeMetrics(contracts, obligations,
...
- `metrics.obligations_at_risk`: obligations with `status === "at_risk"`
  (strict — matches the retired `content/busa-clm-app/server/demo.ts`'s inline metric).
...
tile above, ported verbatim from the retired `content/busa-clm-app/app.js`:
